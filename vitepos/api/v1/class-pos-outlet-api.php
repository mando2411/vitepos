...
class Pos_Outlet_Api extends API_Base {
	...
	public function routes() {
		...
		$this->register_rest_route( 'GET', 'end-of-day-data/(?P<id>\d+)', array( $this, 'end_of_day_data' ) );
		$this->register_rest_route( 'GET', 'cash-drawer-tips/(?P<id>\d+)', array( $this, 'drawer_tips' ) );
		$this->register_rest_route( 'POST', 'user-tips-log', array( $this, 'user_tips_log' ) );
		$this->register_rest_route( 'POST', 'withdraw-tips', array( $this, 'withdraw_tips_from_drawer' ) );
	...
	public function set_route_permission( $route ) {
		switch ( $route ) {
			case 'list':
				return current_user_can( 'pos-menu' ) || current_user_can( 'table-menu' );
			default:
	...
	public function get_cash_drawer_info() {
		$cash_drawer = Mapbd_Pos_Cash_Drawer::get_by_counter(
			$this->get_outlet_id(),
			$this->get_counter_id(),
	...
	public function cash_drawer_log() {
		...
		if ( current_user_can( 'drawer-log' ) ) {
			...
			if ( $response_data->set_total_records( $mainobj->count_all() ) ) {
				...
				$response_data->rowdata = $mainobj->select_all_grid_data(
					'',
					$order_by,
					$order,
					$response_data->limit,
					$response_data->limit_start()
				);
	...
	/**
	 * The end of day data is generated by appsbd
	 *
	 * @param mixed $data It is data param.
	 */
	public function end_of_day_data( $data ) {
		if ( ! empty( $data['id'] ) ) {
			$data_summary = Mapbd_Pos_Cash_Drawer_Types::get_order_summary_by_types( $data['id'] );
			$withdrawn    = Mapbd_Pos_Cash_Drawer::get_withdrawn_amount_by_id( $data['id'] );
			if ( ! empty( $withdrawn ) && $withdrawn > 0 ) {
				$obj               = new \stdClass();
				$obj->payment_type = "W";
				$obj->title        = "Withdrawn";
				$obj->total        = $withdrawn;
			...
			if ( $closed_drawer->select() ) {
				if ( 'Y' === POS_Settings::get_module_instance()->get_option( 'drawer_counted_amount', 'N' ) ) {
					...
					$this->set_response( true, '', [ 'data' => $data_summary, 'counted_amount' => $counted_amount ] );
				} else {
					$this->set_response( true, '', [ 'data' => $data_summary ] );
				}
			}
	 ...
	 * @return \Appsbd\V1\libs\API_Response
	 */
	public function drawer_tips( $data ) {
		if ( ! empty( $data['id'] ) ) {
			$id      = intval( $data['id'] );
			$log_obj = new Mapbd_Pos_Tips_Log();
			$log_obj->cashdrawer_id( $id );
			$data = $log_obj->select_all_grid_data();
			foreach ( $data as &$log ) {
				if ( ! empty( $log->user_to ) ) {
					if ( $log->user_to == $this->get_current_user_id() ) {
						$log->user_to_name = 'Me';
					} else {
						$log->user_to_name = $this->get_user_name_by_id( $log->user_to );
					}
				}
				if ( ! empty( $log->user_by ) ) {
					if ( $log->user_by == $this->get_current_user_id() ) {
						$log->user_by_name = 'Me';
					} else {
						$log->user_by_name = $this->get_user_name_by_id( $log->user_by );
					}
				}
				if ( ! $log->msg ) {
					$log->user_note = '';
	...
	public function user_tips_log() {
		$response = new Ajax_Data_Response();
		$user_id  = $this->get_payload( 'id' );
		if ( ! empty( $user_id ) ) {
			$limit           = $this->get_payload( 'limit' );
			$page            = $this->get_payload( 'page' );
			$src_props       = $this->get_payload( 'src_by', array() );
			...
			$id              = intval( $user_id );
			$log_obj         = new Mapbd_Pos_Tips_Log();
			$log_obj->set_search_by_param( $src_props );
			$log_obj->user_to( $id );
			$log_obj->set_sort_by_param( $sort_props );
			$records = $log_obj->count_all();
			if ( $response->set_grid_records( $records ) ) {
				$all_log_data_obj = new Mapbd_Pos_Tips_Log();
				$all_log_data_obj->set_search_by_param( $src_props );
				$all_log_data_obj->user_to( $id );
				$all_log_data_obj->set_sort_by_param( $sort_props );
				if ( $all_log_data_obj->count_all() > 0 ) {
					$all_data = $all_log_data_obj->select_all_grid_data( 'outlet_id,amount,entry_date,msg,prev_amount,ref_val,type,user_by,user_to',
						'id', 'DESC', 0 );
					foreach ( $all_data as &$log ) {
						if ( ! empty( $log->user_to ) ) {
							if ( $log->user_to == $this->get_current_user_id() ) {
								$log->user_to_name = 'Me';
							} else {
						...
						if ( ! empty( $log->outlet_id ) ) {
							$log->outlet_name = Mapbd_Pos_Warehouse::find_by( 'id', $log->outlet_id )->name;
						}
					}
					$offset = ( $page - 1 ) * $limit;
				...
				$response->final_response->alldata = array_reverse( $all_data );
			}
		}
	...
	/**
	 * The withdraw tips from drawer is generated by appsbd
	 */
	public function withdraw_tips_from_drawer() {
		...
		$given_to  = $this->get_payload( 'given_to', '' );
		if ( empty( $given_to ) ) {
			$this->add_error( "Invalid user for tips withdrawal" );
			$this->set_response( false );

			return $this->response->get_response();
		}
		$access_outlets = get_user_meta( $given_to, 'outlet_id', true );
		if ( ! empty( $access_outlets ) && ! in_array( $outlet, $access_outlets ) ) {
			$this->add_error( "This user is not allowed to withdraw tips from this outlet" );
			$this->set_response( false );
		...
		if ( ! Mapbd_Pos_Cash_Drawer::add_cash_entry(
			'C',
			'Tips Cash Withdrawn',
			$amount,
			$cash_drawer,
			...
			$info                = new \stdClass();
			$info->cashdrawer_id = $cash_drawer->id;
			$info->amount        = $amount;
			$info->outlet_id     = $outlet;
			$logs                = Mapbd_Pos_Cash_Drawer_Log::find_all_by(
				'cash_drawer_id',
				$cash_drawer->id,
				array(),
				'id',
			...
			$info->ref_val       = $logs[0]->id;
			/**
			 * Its for check is there any change before process
			 *
			 * @since 3.1.8
			 */
			do_action( 'appsbd/vitepos/action/withdraw-user-tips', $info, $given_to );
			$cash_drawer = Mapbd_Pos_Cash_Drawer::find_by( 'id', $cash_drawer->id );
	...
	public function get_drawer_info( $cash_drawer ) {
		...
		$return_obj->tips_summary    = Mapbd_Pos_Tips_Log::total_tips_by_cash_drawer( $cash_drawer->id );
