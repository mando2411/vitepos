...
class Pos_Restaurant_Api extends API_Base {
	...
	public function routes() {
		...
		$this->register_rest_route( 'POST', 'picked-to-kitchen', array( $this, 'send_picked_to_kitchen' ) );
		$this->register_rest_route( 'POST', 'pick-and-send', array( $this, 'pick_and_send_to_kitchen' ) );
		$this->register_rest_route( 'POST', 'table-order-list', array( $this, 'table_order_list' ) );
		$this->register_rest_route( 'POST', 'basic-remove-item', array( $this, 'basic_remove_item' ) );
	}
	...
	public function set_route_permission( $route ) {
		switch ( $route ) {
			case ( 'restaurant-payment' ):
				return current_user_can( 'pos-menu' ) || current_user_can( 'cashier-menu' ) || current_user_can( 'basic-pos' );
			case ( 'canned-message' ):
			...
			case ( 'send-to-kitchen' ):
				return current_user_can( 'waiter-to-kitchen' ) || current_user_can( 'pos-menu' ) || current_user_can( 'cashier-menu' ) || current_user_can( 'basic-pos' );
	...
	public function cancel_order() {
		...
		if ( ! empty( $order_id ) ) {
			...
			if ( in_array( $status, array( 'vt_kitchen_deny', 'vt_in_kitchen', 'vt_processing' ) ) ) {
				...
				if ( $order->update_status( 'cancelled', 'Order Cancel', true ) ) {
					...
					if ( 'B' == POS_Settings::get_pos_mode() ) {
						$this->add_time_by_status( $order, 'vt_cashier_cancel' );
					}
		...
		if ( ! empty( $order_id ) ) {
			...
			if ( $new_order->deny_item( $order_id, $item_id, $reason_id ) ) {
				...
				if ( $this->get_is_same_status( $updated_order->items, 'vt_it_denied' ) ) {
					...
					$msg_obj->id( $reason_id );
					if ( $msg_obj->select( 'msg' ) ) {
						$updated_order = $this->update_order_status_by_item(
							$order,
							'vt_kitchen_deny',
							'Deny from kitchen',
							$msg_obj->msg
	...
	public function update_order() {
		...
		if ( ! empty( $order_id ) ) {
			...
			$is_item_added = false;
			$order         = new \WC_Order( $order_id );
			if ( $order->get_meta( '_vt_is_paid' ) == 'Y' ) {
				$this->response->set_response( false, 'You can not update paid order.', null );

				return $this->response->get_response();
			}

			if ( $order->get_meta( '_vtp_is_item_wise' ) != 'Y' && 'R' == POS_Settings::get_pos_mode() ) {
				$this->response->set_response( false, 'Order is not item wise', null );

				return $this->response->get_response();
			}
			if ( $order->get_status() == 'vtu_order_picked' ) {
				/**
				 * Its for check is there any change before process
				 *
				 * @since 1.0
				 */
				do_action( 'appsbd/vitepos-user/action/update-item-wise-status', $order_id, 'vt_it_kitchen' );
			}
			if ( $new_order->add_item( $order_id, $items, $is_item_added ) ) {
				...
				if ( $is_item_added ) {
					...
					} elseif ( 'B' == POS_Settings::get_pos_mode() && $this->get_is_same_status( $updated_order->items, '' ) ) {
						$status = 'vt_processing';
						$order->update_status( $status, 'New item has been sent for processing' );
					} else {
						$status = 'vt_preparing';
						$order->update_status( $status, 'New item has been sent to kitchen' );
					}
	...
	public function update_user_order() {
		...
		if ( ! empty( $order_id ) ) {
			...
			if ( $new_order->add_item( $order_id, $items, $is_item_added ) ) {
				$updated_order = POS_Order::get_from_woo_order_restro_by_id( $order_id, false, true );
				if ( $is_item_added ) {
					$status = 'vtu_order_placed';
					if ( $this->get_is_same_status( $updated_order->items, 'vt_it_placed' ) ) {
						$order->update_status( $status, 'New item has been sent to kitchen' );
					} elseif ( $this->get_is_same_status( $updated_order->items, 'vt_it_kitchen' ) ) {
						$order->update_status( $status, 'New item has been sent to kitchen' );
					} else {
						$status = 'vt_preparing';
						$order->update_status( $status, 'New item has been sent to kitchen' );
				...
				if ( $new_order->remove_item( $order_id, $item_id ) ) {
					...
					if ( $this->get_is_same_status( $updated_order->items, 'vt_it_kitchen' ) ) {
						$updated_order = $this->update_order_status_by_item(
							$order,
							'vt_in_kitchen',
							'Item removed and status changed',
							'Item removed and status changed'
						);
					} elseif ( $this->get_is_same_status( $updated_order->items, 'vt_it_placed' ) ) {
						$updated_order = $this->update_order_status_by_item(
							$order,
							'vtu_order_placed',
							'Item removed and status changed',
					...
					} elseif ( $this->get_is_same_status( $updated_order->items, 'vt_it_picked' ) ) {
						$updated_order = $this->update_order_status_by_item(
							$order,
							'vtu_order_picked',
							'Item removed and status changed',
						...
						$next_status = $new_order->next_status_to_update(
							$order_id,
							array( 'vt_it_preparing', 'vt_it_ready', 'vt_it_served', 'vt_it_denied' )
						);
						if ( ! empty( $next_status ) && $order->get_status() != $next_status ) {
							$updated_order = $this->update_order_status_by_item(
								$order,
								$next_status,
								'Order status changed after removed item',
								'Order status changed due to remove item'
							);
						}
					}
					/**
					...
					$this->response->set_response( false, 'Item remove failed' );
	...
	/**
	 * The basic remove item is generated by appsbd
	 */
	public function basic_remove_item() {
		$order_id = $this->get_payload( 'order_id' );
		$item_id  = $this->get_payload( 'item_id' );
		if ( ! empty( $order_id ) ) {
			$new_order = new POS_Restro_Payment( $this->payload, $this->get_outlet_id(), $this->get_counter_id() );
			...
			if ( count( $order->get_items() ) > 1 && POS_Settings::can_basic_item_remove() ) {
				if ( $new_order->remove_item( $order_id, $item_id ) ) {
					$updated_order = POS_Order::get_from_woo_order_restro_by_id( $order_id, false, true );
					if ( $this->get_is_same_status( $updated_order->items, '' ) ) {
						$updated_order = $this->update_order_status_by_item(
							$order,
							'vt_processing',
							'Item removed and status changed',
	...
	public function item_cancel_req_ans() {
		...
		if ( ! empty( $order_id ) && ! empty( $ans ) ) {
			...
			} elseif ( $new_order->deny_cancel_item( $order_id, $item_id ) ) {
				$updated_order = POS_Order::get_from_woo_order_restro_by_id( $order_id, false, true );
	...
	public function complete_item() {
		...
		if ( ! empty( $order_id ) ) {
			...
			if ( $order->get_status() == 'vt_preparing' ) {
				...
				if ( $this->get_is_same_status(
					$updated_order->items,
					'vt_it_ready'
				) || $this->can_go_next_status(
					$updated_order->items,
					array( 'vt_it_ready', 'vt_it_served' )
				) ) {
					$updated_order = $this->update_order_status_by_item(
						$order,
						'vt_ready_to_srv',
	 ...
	 * @return \Appsbd\V1\libs\API_Response
	 */
	public function send_picked_to_kitchen() {
		self::set_vite_pos_request();
		$order_id = $this->get_payload( 'order_id', '' );
		$order    = wc_get_order( $order_id );
		$order->update_meta_data( '_vtp_is_resto', 'Y' );
		if ( $order->get_meta( '_vtp_is_item_wise' ) == 'Y' ) {
			$items = $order->get_items();
			foreach ( $items as $oitem ) {
				$oitem->update_meta_data( '_vtp_item_status', 'vt_it_kitchen' );
		...
		if ( $order->update_status( 'vt_in_kitchen', 'Sent to kitchen', true ) ) {
			...
			$this->response->set_response( true, '', $order_details );
	 ...
	 * @return \Appsbd\V1\libs\API_Response
	 */
	public function pick_and_send_to_kitchen() {
		self::set_vite_pos_request();
		$order_id  = $this->get_payload( 'order_id', '' );
		$waiter_id = $this->get_payload( 'waiter_id', '' );
		if ( empty( $waiter_id ) ) {
			$waiter_id = $this->get_current_user_id();
		}
		$picked_order = POS_Order::pick_user_app_order( $order_id, $waiter_id );

		if ( ! empty( $picked_order ) ) {
			$order_id = $this->get_payload( 'order_id', '' );
			$order    = wc_get_order( $order_id );
			...
			if ( $order->update_status( 'vt_in_kitchen', 'Order successfully picked and sent to kitchen', true ) ) {
				$order->save();
				$this->add_time_by_status( $order, 'vt_in_kitchen' );
				POS_Settings::get_module_instance()->add_info( 'Order successfully picked and sent to kitchen' );
				$order_details = POS_Order::get_from_woo_order_restro_by_id(
					$order_id,
					false,
					true
	...
	/**
	 * The table order list is generated by appsbd
	 */
	public function table_order_list() {
		$response_data = new API_Data_Response();
		$table_id      = $this->get_payload( 'id', null );
		if ( ! empty( $table_id ) ) {
			$args = array(
				'status'        => array( 'completed', 'vt_processing' ),
				'limit'         => $this->get_payload( 'limit', 10 ),
				'page'          => $this->get_payload( 'page', 1 ),
				'orderby'       => 'date',
				'order'         => 'DESC',
				...
				'date_query'    => array(
					array(
						'after'     => gmdate( 'Y-m-d H:i:s', strtotime( '-24 hours' ) ),
						'inclusive' => true,
					),
				),
			);
	...
	public function change_order_table() {
		...
		if ( $order ) {
			...
			if ( ! empty( $tables ) ) {
				if ( vitepos_wc_update_meta( $id, '_vtp_tables', $tables ) && vitepos_wc_update_meta(
					$id,
					'_vtp_persons',
					$person
				) && vitepos_wc_update_meta( $id, '_vtp_order_type', $order_type ) ) {
					if ( POS_Settings::get_pos_mode() == 'B' ) {
						if ( ! empty( $waiter_id ) && ! vitepos_wc_update_meta( $id, '_vtp_order_by', $waiter_id ) ) {
							$this->response->set_response( false, 'Waiter not updated', null );
