...
class Pos_Order_Api extends API_Base {
	...
	public function routes() {
		...
		$this->register_rest_route( 'POST', 'make-user-app-payment', array( $this, 'make_user_app_payment' ) );
		...
		$this->register_rest_route( 'POST', 'app-order-list', array( $this, 'app_order_list' ) );
		$this->register_rest_route( 'POST', 'pick-order', array( $this, 'pick_order' ) );
		$this->register_rest_route( 'GET', 'app-order-details/(?P<id>\d+)', array( $this, 'app_order_details' ) );
	}
	...
	private function make_order_payment( $is_offline = false ) {
		...
		if ( ! empty( $order_id ) && 'B' != POS_Settings::get_pos_mode() ) {
			if ( $payment->order_checkout( $order_id ) ) {
				$this->response->set_response( true, '', $payment->get_order_details() );
			} else {
				$this->response->set_response( false, '', $payment->get_order_details() );
			}
		} elseif ( 'P' == POS_Settings::get_pos_mode() ) {
			if ( $payment->restaurant_checkout_pay_first( $is_offline ) ) {
				$this->response->set_response( true, '', $payment->get_order_details() );
			} else {
				$this->response->set_response( false, '', $payment->get_order_details() );
			}
		} elseif ( 'B' == POS_Settings::get_pos_mode() ) {
			if ( ! empty( $order_id ) ) {
				$prev_order = wc_get_order( $order_id );
				$is_vitepos = $prev_order->get_meta( '_is_vitepos' );
				if ( 'vt_processing' == $prev_order->get_status() && 'completed' == $this->payload['status'] && 'Y' == $is_vitepos ) {
					$payment->restaurant_checkout( $order_id, $is_offline );
					$this->response->set_response( true, '', $payment->get_order_details() );
				} else {
					$this->response->set_response( false, 'Order status mismatch', $payment->get_order_details() );
				}
			} elseif ( $payment->send_to_kitchen( $is_offline ) ) {
					$this->response->set_response( true, '', $payment->get_order_details() );
			} else {
				$this->response->set_response( false, '', $payment->get_order_details() );
			}
	...
	public function order_list() {
		...
		$args          = array(
			'status'        => array( 'completed', POS_Settings::def_oder_status(), 'pending' ),
	 ...
	 * @return \Appsbd\V1\libs\API_Response
	 */
	public function app_order_list() {
		$response_data = new API_Data_Response();
		$args          = array(
			'limit'    => $this->get_payload( 'limit', 10 ),
			'page'     => $this->get_payload( 'page', 1 ),
	...
	public function pick_order() {
		...
		if ( $picked_order ) {
			 ...
			 * @since 2.0
			 */
			do_action( 'vitepos/action/send-order-push', $picked_order );
			$this->response->set_response( true, '', $picked_order );

			return $this->response;
		}
		POS_Settings::get_module_instance()->add_error( 'Order pick failed' );
		return $this->response;
	}
	 ...
	 * @return \Appsbd\V1\libs\API_Response
	 */
	public function app_order_details( $data ) {
		if ( ! empty( $data['id'] ) ) {
			$id    = intval( $data['id'] );
			$order = wc_get_order( $id );
			if ( ! empty( $order ) ) {
				$order_data         = POS_Order::get_from_woo_order_details( $order, true );
				$order_data->status = $order->get_status();
				$this->response->set_response( true, 'Order Found', $order_data );
				return $this->response;
			} else {
	...
	public function order_refund() {
		...
		if ( ! empty( $order_id ) ) {
			...
			if ( ! empty( $order ) && $order instanceof \WC_Order ) {
				...
				$factor = vitepos_wc_amount( $can_refund_amount - $refund_amount );
				$re_discount = (float) $this->get_payload( 're_discount', 0 );
				$re_fee = (float) $this->get_payload( 're_fee', 0 );
				$sign = 'P';
				if ( $factor < 0.0 ) {
					$sign = 'N';
					$factor = ( -1 ) * ( $factor );
				...
				if ( is_wp_error( $refund ) ) {
					...
					$refund->add_meta_data( '_vtp_refund_fee', $re_fee );
					$refund->save();
					...
					if ( $re_fee > 0 ) {
						...
						if ( $order->meta_exists( '_vtp_re_fee' ) ) {
							$pre_fee = $order->get_meta( '_vtp_re_fee' );
						}
						$re_fee += appsbd_wc_amount( $pre_fee );
						$order->update_meta_data( '_vtp_re_fee', $re_fee );
					}
					...
					$refund_return->stocks = Vitepos_Stock_Manage::get();
		...
		if ( $order ) {
			if ( $order->get_status() == 'vt_in_kitchen' ) {
				...
				if ( ! empty( $msg ) ) {
					...
					$order->add_order_note( 'Order status changed from ' . "{$outlet_obj->name}" );
				}
				$order->update_meta_data( '_vtp_processed_by', $processed_by );
				$order->update_meta_data( '_vtp_outlet_id', $this->get_outlet_id() );
				if ( $order->update_status( $this->get_payload( 'status' ), 'Order status has been changed' ) ) {
					$data               = new \stdClass();
					$order_data         = POS_Order::get_from_woo_order_details( $order );
