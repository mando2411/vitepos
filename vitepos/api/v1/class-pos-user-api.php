...
class Pos_User_Api extends API_Base {
	...
	public function routes() {
		...
		$this->register_rest_route( 'GET', 'basic-waiter-list', array( $this, 'basic_waiter_list' ) );
	...
	private function get_logged_user_response( $user ) {
		...
		if ( ! empty( $response_data->caps['cashier-menu'] ) && POS_Settings::get_pos_mode() == 'B' ) {
			$response_data->caps['basic-pos'] = true;
		}
		$response_data->outlets = Mapbd_Pos_Warehouse::get_outlet_details( $user );
		/**
		 * Its for filter the outlets and counter
		 *
		 * @since 3.1.9
		 */
		$response_data->outlets      = apply_filters(
			'apbd-vitepos/filter/user-outlet',
			$response_data->outlets,
			$user
		);
			...
			if ( $this->payload['user_id'] ) {
				if ( current_user_can( 'change-any-user-pass' ) ) {
					...
					if ( ! empty( $user_data->ID ) ) {
						 ...
						 */
						do_action(
							'apbd-vtpos/action/send-temp-password-email',
							$user_data,
							$this->payload['newPass']
						);
	...
	public function roles() {
		...
		$response_roles = apply_filters( 'appsbd/vitepos/filter/roles', $response_roles );
	...
	public function user_list() {
		...
		$roles = apply_filters( 'appsbd/vitepos/filter/roles', $roles );
		$args  = array(
	...
	/**
	 * The basic waiter list is generated by appsbd
	 */
	public function basic_waiter_list() {
		$response_data = new API_Data_Response();
		$src_props     = $this->get_payload( 'src_by', array() );
		$sort_by_props = $this->get_payload( 'sort_by', array() );
		$args          = array(
		...
		$response_user = array();
		foreach ( $users as $user ) {
			if ( user_can( $user->ID, 'waiter-menu' ) ) {
				$waiter = new \stdClass();
				$total_user ++;
				$waiter->id        = $user->ID;
				$waiter->name      = $user->first_name ? $user->first_name . ' ' . $user->last_name : $user->user_nicename;
				$waiter->email     = $user->user_email;
				$waiter->outlet_id = get_user_meta( $user->ID, 'outlet_id', true );
				$waiter->image     = get_user_meta(
					$user->ID,
					'_vtpos_user_img',
					true
	...
	public function outlet_panel() {
		...
		$existing_drawer              = Mapbd_Pos_Cash_Drawer::get_by_counter(
			$outlet_place->outlet,
			$outlet_place->counter,
			$this->get_current_user_id()
		);
		$outlet_place->cd_balance     = ! empty( $existing_drawer->closing_balance ) ? $existing_drawer->closing_balance : 0;
		$closing_balance              = ! empty( $existing_drawer->closing_balance ) ? $existing_drawer->closing_balance : 0;
		$outlet_place->cash_drawer_id = ! empty( $existing_drawer->id ) ? $existing_drawer->id : 0;
		$outlet_place->is_submitted   = $this->payload['is_submitted'] != 0;
		if ( $outlet_place->is_new ) {
			$outlet_place->cd_balance     = (float) $this->get_payload( 'cd_balance' );
			$outlet_place->counted_amount = (float) $this->get_payload( 'counted_amount' );
			if ( 'Y' === POS_Settings::get_module_instance()->get_option( 'drawer_counted_amount', 'N' ) ) {
				if ( 'Y' == POS_Settings::get_module_instance()->get_option( 'is_required_drawer_counted_amount',
						'N' ) ) {
					if ( ! isset( $outlet_place->counted_amount ) ) {
						$this->add_error( 'Invalid counted amount' );
						$this->set_response( false );

						return $this->response->get_response();
					}
					if ( $closing_balance > 0.00 && $outlet_place->counted_amount <= 0 ) {
						$this->add_error( 'Counted amount is required' );
						$this->set_response( false );

						return $this->response->get_response();
					}
				}
				if ( ! empty( $outlet_place->counted_amount ) && $outlet_place->counted_amount < 0 ) {
					$this->add_error( 'invalid counted amount' );
					$this->set_response( false );

					return $this->response->get_response();
				}
				if ( empty( $outlet_place->counted_amount ) ) {
					$outlet_place->counted_amount = 0.00;
				}

			}
			$cash_drawar = Mapbd_Pos_Cash_Drawer::create_by_counter(
				$outlet_place->cd_balance,
				$outlet_place->outlet,
				$outlet_place->counter,
				$this->get_current_user_id(),
				$outlet_place->counted_amount
			);
	...
	public function close_cash_drawer() {
		$closed_amount = (float) $this->get_payload( 'counted_amount' );
		...
		if ( 'Y' === POS_Settings::get_module_instance()->get_option( 'drawer_counted_amount', 'N' ) ) {
			if ( 'Y' == POS_Settings::get_module_instance()->get_option( 'is_required_drawer_counted_amount', 'N' ) ) {
				if ( ! isset( $closed_amount ) ) {
					$this->add_error( 'Invalid counted amount' );
					$this->set_response( false );

					return $this->response->get_response();
				}
				$current_balance = $existing_drawer->closing_balance;
				if ( $current_balance > 0.00 && $closed_amount <= 0 ) {
					$this->add_error( 'Counted amount is required' );
					$this->set_response( false );
			...
			if ( ! empty( $closed_amount ) && $closed_amount < 0 ) {
				$this->add_error( 'invalid counted amount' );
				$this->set_response( false );

				return $this->response->get_response();
			}
			if ( empty( $closed_amount ) ) {
				$closed_amount = 0.00;
			}

		}
		if ( ! empty( $existing_drawer ) && $existing_drawer->set_close_drawer( '', $closed_amount ) ) {
	...
	public function create_user() {
		if ( ! empty( $this->payload ) ) {
			...
			if ( ! empty( $old_cus ) ) {
				$key = $old_cus->ID . $old_cus->user_nicename . date( 'm' );
				if ( ! wp_verify_nonce( $this->get_payload( 'nonce' ), $key ) ) {
				...
				$is_max = apply_filters( 'appsbd/vitepos/filter/is-max-user', false );
				if ( $is_max ) {
					$this->add_error( 'User creation limit exceeded' );
					$this->response->set_response( false, '' );
