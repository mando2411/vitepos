...
class Pos_Table_Api extends API_Base {
	...
	public function table_list() {
		...
		$all_props           = 'title,id,outlet_id';
		$mainobj->set_search_by_param( $src_props, $all_props );
		$mainobj->outlet_id( $this->get_outlet_id() );
	...
	public function table_details( $data ) {
		if ( ! empty( $data['id'] ) ) {
			...
			$main_obj->id( $id );
			if ( $main_obj->select( 'id,added_by,status,assigned_waiters,outlet_id,is_mergeable,is_reserved,image,seat_cap,des,type,title' ) ) {
				unset( $main_obj->setted_propertyfor_log );
	...
	public function create_table() {
		...
		if ( $table_obj->is_valid_form( true ) ) {
			if ( $table_obj->save() ) {
				...
				$table_obj->image = intval( $table_obj->image );
				if ( ! empty( $table_obj->image ) ) {
					$table_obj->image = wp_get_attachment_url( $table_obj->image );
				}
				$this->set_response( true, '', $table_obj );
			} else {
		...
		if ( $old_object ) {
			...
			if ( $table_obj->set_from_array( $this->payload, false ) ) {
				...
				if ( $is_updated ) {
					...
					$updated_obj->image = intval( $updated_obj->image );
					if ( ! empty( $updated_obj->image ) ) {
						$updated_obj->image = wp_get_attachment_url( $updated_obj->image );
					}
