...
class POS_Role extends Vitepos_Module {
	...
	public function new_role_added( $role ) {
		if ( $role instanceof Mapbd_pos_role ) {
			...
			$is_editable='Y' == $role->is_editable && ! isset( $existing_roles[ $role->slug ] );
			$is_editable=apply_filters('appsbd/vitepos/filter/is-editable',$is_editable,$role);
			if ( $is_editable) {
				add_role(
					$role->slug,
					$role->name,
	...
	public function default_resources( $resources ) {
		...
		if ( POS_Settings::get_pos_mode() == 'B' ) {
			$resources[] = ACL_Resource::get_resource( 'basic-pos', 'Basic POS', '01. POS' );
		} else {
		...
		$resources[] = ACL_Resource::get_resource( 'pos-tips', 'Order Tips', '01. POS', '' );
		...
		$resources[] = ACL_Resource::get_resource(
			...
			'Manage Barcode Page',
			'09. Barcode',
			'This role user can create,update and delete customize barcode page '
		);
		...
		$resources[] = ACL_Resource::get_resource(
			...
			'10. Drawer Log',
			'This roles user can see cash drawer log of any outlet'
		);
		$resources[] = ACL_Resource::get_resource(
			'close-drawers',
			'Close any drawer',
		...
		if ( POS_Settings::is_restaurant_mode() ) {
			...
			if ( POS_Settings::get_pos_mode() == 'R' || POS_Settings::get_pos_mode() == 'B' ) {
				
				$resources[] = ACL_Resource::get_resource(
					'waiter-menu',
					'Waiter Menu',
					'12. Waiter Panel',
					'This roles user can see waiter panel'
				);
			}
			...
			if ( POS_Settings::get_pos_mode() == 'R' ) {
				$resources[] = ACL_Resource::get_resource(
					...
					'Serve Order',
					'12. Waiter Panel',
					'This roles user can make order status to served'
				);
				$resources[] = ACL_Resource::get_resource(
					'cancel-waiter-order',
					'Cancel Order',
				...
				$resources[] = ACL_Resource::get_resource(
					...
					'Send To Kitchen',
					'12. Waiter Panel',
					'This roles user can make order'
				);
			}
			...
			if ( POS_Settings::get_pos_mode() != 'B' ) {
				...
				$resources[] = ACL_Resource::get_resource(
					...
					'Start Preparing',
					'14. Kitchen Panel',
					'This roles user can deny order from restaurant kitchen panel'
				);
				$resources[] = ACL_Resource::get_resource(
					'deny-order',
					'Deny Order',
				...
				$resources[] = ACL_Resource::get_resource(
					...
					'Ready To Serve',
					'14. Kitchen Panel',
					'This roles user can make ready to serve from restaurant kitchen panel'
				);
				$resources[] = ACL_Resource::get_resource(
					'accept-cancel',
					'Accept Cancel Request',
					'14. Kitchen Panel',
					'This roles user can accept cancel request'
				);
				$resources[] = ACL_Resource::get_resource(
					'deny-cancel',
					'Deny Cancel Request',
					'14. Kitchen Panel',
					'This roles user can deny cancel request'
				);
				$resources[] = ACL_Resource::get_resource(
					'print-order-kitchen',
					'Print Kitchen Order',
					'14. Kitchen Panel',
					'This roles user can print order details from restaurant kitchen panel'
				);
			}

			if ( POS_Settings::get_pos_mode() == 'P' && POS_Settings::is_kitchen() ) {
				$resources[] = ACL_Resource::get_resource(
					'make-complete-kitchen',
					'Complete Order',
					'14. Kitchen Panel',
					'This roles user can make complete an order'
				);
			}
			
			$resources[] = ACL_Resource::get_resource( 'table-menu', 'Table Menu', '15. Table Panel', 'This roles user can see table panel' );
			$resources[] = ACL_Resource::get_resource( 'table-add', 'Table Add', '15. Table Panel', '' );
			$resources[] = ACL_Resource::get_resource( 'table-edit', 'Table Edit', '15. Table Panel', '' );
			$resources[] = ACL_Resource::get_resource( 'table-delete', 'Table Delete', '15. Table Panel', '' );
			$resources[] = ACL_Resource::get_resource( 'table-barcode', 'Table Barcode', '15. Table Panel', '' );
		}
		
		$resources[] = ACL_Resource::get_resource( 'report-menu', 'Report Panel', '16. Report Panel', '' );
		$resources[] = ACL_Resource::get_resource( 'report-order', 'Orders Report', '16. Report Panel', '' );
		$resources[] = ACL_Resource::get_resource( 'report-product', 'Products Report', '16. Report Panel', '' );
		$resources[] = ACL_Resource::get_resource( 'report-customer', 'Customers Report', '16. Report Panel', '' );
		$resources[] = ACL_Resource::get_resource( 'report-staff', 'Staff Report', '16. Report Panel', '' );
