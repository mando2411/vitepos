...
class MU_Plugin_Settings extends Vitepos_Module {
			...
			if ( is_plugin_active( $plugin ) ) {
				...
				if ( 'Y' == $target_status ) {
					if ( ! in_array( $plugin, $mu_skipped ) ) {
						...
						$this->add_info( 'Success ' );

						return $res->display_with_response( true, $target_status );
					}
				...
				if ( update_option( '_vt_mu_skipped', $mu_skipped ) ) {
					...
					$this->add_error( 'Process failed' );

					return $res->display_with_response( false );
				}
			} else {
				$this->add_error( 'Inactivate plugin' );
			}
		}
	...
	public function get_non_skippable_plugins() {
		 ...
		 * @since 3.2.4
		 */
		$appsbd_plugins = apply_filters( 'appsbd/plugins', array() );
		$non_skipable   = array(
	...
	public function data() {
		...
		foreach ( $active_plugins as $active_plugin ) {
			...
			if ( in_array( $active_plugin, $pre_skipped ) ) {
				continue;
				$plugin_response_data->pre_skipped = 'Y';
				$plugin_response_data->is_skipped  = 'Y';
			} else {
