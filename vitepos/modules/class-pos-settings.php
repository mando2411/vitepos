...
class POS_Settings extends Vitepos_Module {
	...
	public function initialize() {
        ...
        $this->update_cogs_from_vt_purchase_cost();
	}
	...
	public static function do_version_update( $current_version, $previous_version, $is_force = false ) {
		...
		if ( $is_force || version_compare( $previous_version, '3.4.1', '<' ) ) {
           self::update_cogs_from_vt_purchase_cost();
			...
			if ( !empty( $custom_methods ) ) {
				...
				foreach ($custom_methods as &$custom_method) {
					if ($custom_method->id === 'W') {
						foreach ($needle as $key => $value) {
							if (!in_array($value, $existing_ids)) {
								$custom_method->id = $value;
								$new_val = $value;
								break;
							}
						}
				...
				try {
					if (!empty($new_val))
					{
						if(Payment_Custom_Methods::update_custom_methods( $custom_methods )){
							$obj = new Mapbd_Pos_Cash_Drawer_Types();
							$obj->payment_type($new_val);
							$obj->set_where_update('payment_type','W');
							$obj->update(true);
						}
					}
				} catch (\Throwable $th) {
    ...
    public static function update_cogs_from_vt_purchase_cost(){
        $prefix = Mapbd_Pos_Cash_Drawer_Log::get_db_object()->prefix;        
        $query  = "select * from  {$prefix}postmeta where meta_key='_vt_purchase_cost'";
        $results=Mapbd_Pos_Cash_Drawer_Log::get_db_object()->get_results( $query );       
	    foreach ( $results as $result ) {
            $exits= get_post_meta($result->post_id,'_cogs_total_value',true);		  
            if($exits==""){
                if(update_post_meta($result->post_id,'_cogs_total_value',$result->meta_value)){	             
                    delete_post_meta($result->post_id,'_vt_purchase_cost');
                }
            }
        }       
    }
	...
	public function on_init() {
		...
		if ( self::is_stockable() ) {
			if ( ! self::is_default_stock() ) {
				if ( self::is_stock_linked_with() ) { 
                    ...
                    add_filter('woocommerce_order_hold_stock_minutes',array($this,'stock_minutes'),90,2);
				}
				add_filter(
        ...
        if(vitepos_is_api_request() || vitepos_is_api_request('vitepos-user/v1/')) {
	        ...
	        add_filter( 'sanitize_title', function( $title, $raw_title, $context ) {
		        if ( preg_match('/[\x{0980}-\x{09FF}]/u', $raw_title ) ) {
	...
	public function stock_minutes( $minutes, $order ) {
        return false;
    }
	...
	public function get_variation_manage_stock( $value, $product ) {
        ...
        if(!$value){
	        if ( $product->get_type() === 'variation' ) {
		        ...
		        return $parent_product->get_manage_stock();
	        }
        }
		return $value;
	}
	...
	public static function get_settings( $type = '' ) {
		...
		$settings->inv_settings->footer_extra = apply_filters('appsbd/vitepos/filter/invoice/footer-extra','');
	...
	public function get_admin_settings() {
        ...
        if ( ! isset( $this->options['gift_receipt'] ) ) {
			$this->options['gift_receipt'] = 'N';
		}
