...
class POS_Settings extends Vitepos_Module {
	...
	public function initialize() {
		...
		add_filter( 'vitepos/filter/load-vitepos-icon', array( $this, 'load_vitepos_icon' ) );
		add_filter( 'vitepos/filter/footer-scripts', array( $this, 'client_footer_scripts' ) );
		add_filter( 'vitepos/filter/assigned-waiters-by-table-id', array( $this, 'get_assigned_waiters' ) );
		...
		add_action( 'vitepos/action/adjust-decimal', array( $this, 'adjust_decimal_for_calculation' ) );
	...
	/**
	 * The adjust decimal for calculation is generated by appsbd
	 */
	public function adjust_decimal_for_calculation() {
		add_filter(
			'wc_get_price_decimals',
			function ( $decimal ) {
				return absint( get_option( 'woocommerce_price_num_decimals', 2 ) ) + 2;
			}
		);
	}

	/**
	 * The load vitepos icon is generated by appsbd
	 *
	 * @return array
	 */
	public function load_vitepos_icon() {
		$links = array(
	 ...
	 * @return bool
	 */
	public static function get_is_order_token() {
		return self::get_module_option( 'is_token_enabled', 'N' ) == 'Y';
	}
	 ...
	 * @return string
	 */
	public static function get_token_prefix() {
		return self::get_module_option( 'token_prefix', '' );
	}
	 ...
	 * @return bool
	 */
	public static function get_token_no() {
		$last_date     = self::get_module_option( 'vtp_last_token_reset_date', '' );
		$reset_time    = self::get_module_option( 'token_reset_time', '00:00:00' );
		$is_resetting  = self::get_module_option( 'enable_token_resetting', 'N' ) == 'Y';
		$token_counter = self::get_module_option( 'vtp_prev_token_count', 0 );
		if ( $is_resetting ) {
			if ( empty( $last_date ) ) {
				$reset_date                                                       = strtotime( current_time( 'Y-m-d' ) . $reset_time );
				self::get_module_instance()->options['vtp_last_token_reset_date'] = $reset_date;
			} else {
				$current_timestamp = current_time( 'timestamp' );
				if ( $current_timestamp >= ( $last_date + 24 * 60 * 60 ) ) {
					$reset_date                                                       = strtotime( current_time( 'Y-m-d' ) . $reset_time );
					$token_counter                                                    = 0;
					self::get_module_instance()->options['vtp_last_token_reset_date'] = $reset_date;
				}
		...
		$up_count                                                    = intval( $token_counter ) + 1;
		self::get_module_instance()->options['vtp_prev_token_count'] = $up_count;
		self::get_module_instance()->update_option();
	...
	/**
	 * The can basic item add is generated by appsbd
	 *
	 * @return bool
	 */
	public static function can_basic_item_add() {
		return 'B' == self::get_pos_mode() && 'Y' == self::get_module_option( 'is_basic_add_item', 'N' );
	}

	/**
	 * The can basic item remove is generated by appsbd
	 *
	 * @return bool
	 */
	public static function can_basic_item_remove() {
		return 'B' == self::get_pos_mode() && 'Y' == self::get_module_option( 'is_basic_remove_item', 'N' );
	}
	...
	public static function do_version_update( $current_version, $previous_version, $is_force = false ) {
		...
		if ( $is_force || version_compare( $previous_version, '1.3', '<' ) ) {
			...
			Mapbd_Pos_Role::db_column_add_or_modify(
				'discount_type',
				'char',
				'1',
				"'P'",
		...
		if ( $is_force || version_compare( $previous_version, '2.0.2', '<' ) ) {
			...
			Mapbd_Pos_Stock_Log::db_column_add_or_modify(
				'stock_type',
				'char',
				'1',
				"'O'",
				'NOT NULL',
				'outlet_id',
				'radio(O=Outlet Wise,W=WooCommerce Stock)'
			);
			Mapbd_Pos_Stock_Log::db_column_add_or_modify(
				'ex1_param',
				'char',
			...
			Mapbd_pos_cash_drawer::db_column_add_or_modify(
				'closing_balance',
				'decimal',
				'11,2',
				'0',
		...
		if ( $is_force || version_compare( $previous_version, '3.0.5', '<' ) ) {
			Mapbd_Pos_Cash_Drawer_Log::db_column_add_or_modify(
				'user_note',
				'varchar',
				'255',
				"''",
        ...
        if ( $is_force || version_compare( $previous_version, '3.2.0', '<' ) ) {
			$query  = "UPDATE {$prefix}postmeta t1 LEFT JOIN {$prefix}postmeta t2 ON t1.post_id = t2.post_id AND t2.meta_key = '_cogs_total_value' SET t1.meta_key = '_cogs_total_value' WHERE t1.meta_key = '_vt_purchase_cost' AND t2.meta_id IS NULL;";
			Mapbd_Pos_Cash_Drawer_Log::get_db_object()->query( $query );
		}

        if ( $is_force || version_compare( $previous_version, '3.3.4', '<' ) ) {
	        Mapbd_Pos_Cash_Drawer::db_column_add_or_modify(
		        'counted_amount',
		        'decimal',
		        '10,2',
		        ...
		        'NULL',
		        'closing_balance',
		        'This is for enter cash drawer ending amount by cashier'
	        );
		}
	}
	...
	public function update_request_option() {
		...
		if ( $is_updated && ( $before_save['token_reset_time'] != $this->options['token_reset_time'] ) ) {
			$time_zone = get_option( 'timezone_string' );
			if ( empty( $timezone ) ) {
				$timezone = 'UTC';
			}
			$date                                       = new \DateTime( 'yesterday', new \DateTimeZone( $timezone ) );
			$prev_day                                   = $date->format( 'Y-m-d' );
			$reset_date                                 = strtotime( $prev_day . $this->options['token_reset_time'] );
			$this->options['vtp_last_token_reset_date'] = $reset_date;
			$this->update_option();
		}
	...
	/**
	 * The get urls is generated by appsbd
	 *
	 * @return string[]|null
	 */
	public static function get_urls() {
		$vitepos_base = get_rest_url( null, 'vitepos/v1' ) . '/';

		$urls = array(
			'sys_login'    => '',
			'heart_bit'    => $vitepos_base . 'system/heart-bit',
			'country_list' => $vitepos_base . 'basic/countries',
			'settings'     => $vitepos_base . 'basic/settings',

			'add_custom_page'    => $vitepos_base . 'basic/add-custom-page',
			'edit_custom_page'   => $vitepos_base . 'basic/edit-custom-page',
			'delete_custom_page' => $vitepos_base . 'basic/delete-custom-page',
			'get_custom_page'    => $vitepos_base . 'basic/get-custom-page',
			...
			'refund_list'          => $vitepos_base . 'order/refund-list',
			'orders_for_refund'    => $vitepos_base . 'order/orders-for-refund',
			'online_list'          => $vitepos_base . 'order/online-list',
			'order_details'        => $vitepos_base . 'order/details',
			'change_status'        => $vitepos_base . 'order/change-status',
			'complete_order'       => $vitepos_base . 'order/complete-order',
			'initials_data'        => $vitepos_base . 'product/initial-data',
			'make_payment'         => $vitepos_base . 'order/make-payment',
			'order_refund'         => $vitepos_base . 'order/order-refund',
			'send_email'           => $vitepos_base . 'order/email',
			'order_cancel'         => $vitepos_base . 'order/order-cancel',
			'order_action'         => $vitepos_base . 'order/order-action',
			'order_complete'       => $vitepos_base . 'order/order-complete',
			'sync_offline_order'   => $vitepos_base . 'order/sync-offline-order',
			'product_details'      => $vitepos_base . 'product/details',
			'create_product'       => $vitepos_base . 'product/create',
			'update_product'       => $vitepos_base . 'product/update',
			'attributes_list'      => $vitepos_base . 'product/attributes',
			'get_stock'            => $vitepos_base . 'product/getStock',
			'scan_product'         => $vitepos_base . 'product/scan-product',
			'vendor_list'          => $vitepos_base . 'vendor/list',
			'vendor_details'       => $vitepos_base . 'vendor/details',
			'create_vendor'        => $vitepos_base . 'vendor/create',
			'update_vendor_status' => $vitepos_base . 'vendor/update_status',
			'delete_vendor'        => $vitepos_base . 'vendor/delete-vendor',
			'delete_customer'      => $vitepos_base . 'customer/delete-customer',
			'delete_user'          => $vitepos_base . 'user/delete-user',
			'waiter_list'          => $vitepos_base . 'user/waiter-list',
			'basic_waiter_list'    => $vitepos_base . 'user/basic-waiter-list',
			'close_cashDrawer'     => $vitepos_base . 'user/close-cash-drawer',
			'delete_product'       => $vitepos_base . 'product/delete-product',
			'make_favorite'        => $vitepos_base . 'product/make-favorite',
			'make_hidden'          => $vitepos_base . 'product/make-hidden',
			'outlet_list'          => $vitepos_base . 'outlet/list',
			'all_outlet_list'      => $vitepos_base . 'outlet/all-outlet-list',
			'cash_drawer_info'     => $vitepos_base . 'outlet/cash-drawer-info',
			'cash_drawer_log'      => $vitepos_base . 'outlet/cash-drawer-log',
			'withdraw_cash'        => $vitepos_base . 'outlet/withdraw-cash',
			'close_drawer'         => $vitepos_base . 'outlet/close-drawer',
			'drawer_log_details'   => $vitepos_base . 'outlet/details',
			'eod_data'             => $vitepos_base . 'outlet/end-of-day-data',
			'drawer_summary'       => $vitepos_base . 'outlet/summary',
			'purchase_list'        => $vitepos_base . 'purchase/list',
			'updated_price_list'   => $vitepos_base . 'purchase/updated-price-list',
			'create_purchase'      => $vitepos_base . 'purchase/create',
			'purchase_details'     => $vitepos_base . 'purchase/details',
			'create_customer'      => $vitepos_base . 'customer/create',
			'check_unique'         => $vitepos_base . 'customer/check-unique',
			'create_user'          => $vitepos_base . 'user/create',
			'user_login'           => $vitepos_base . 'user/login',
			'user_logout'          => $vitepos_base . 'user/logout',
			'change_pass'          => $vitepos_base . 'user/change-pass',
			'change_sound'         => $vitepos_base . 'user/change-sound',
			'change_pass_force'    => $vitepos_base . 'user/change-pass-force',
			'customer_list'        => $vitepos_base . 'customer/list',
			'customerList'         => $vitepos_base . 'customer/customer-list',
			'user_list'            => $vitepos_base . 'user/list',
			'role_list'            => $vitepos_base . 'user/roles',
			'customer_details'     => $vitepos_base . 'customer/details',
			'user_details'         => $vitepos_base . 'user/details',
			'outlet_panel'         => $vitepos_base . 'user/outlet-panel',

			'updated_product_details' => $vitepos_base . 'purchase/updated-product-details',
			'update_product_price'    => $vitepos_base . 'purchase/update-product-price',
			'ignore_update_price'     => $vitepos_base . 'purchase/ignore-update-price',

			
			'category_list'           => $vitepos_base . 'product/categories',
			'all_category_list'       => $vitepos_base . 'product/all-categories',
			'get_all_category_list'   => $vitepos_base . 'product/get-all-categories',
			'add_category'            => $vitepos_base . 'product/add-category',
			'update_category'         => $vitepos_base . 'product/update-category',
			'delete_category'         => $vitepos_base . 'product/delete-category',

			
			'get_category'            => $vitepos_base . 'product/get-category',
			'get_attributes'          => $vitepos_base . 'product/get-attributes',
			'add_attribute'           => $vitepos_base . 'product/add-attribute',
			'get_attribute'           => $vitepos_base . 'product/get-attribute',
			'update_attribute'        => $vitepos_base . 'product/update-attribute',
			'delete_attribute'        => $vitepos_base . 'product/delete-attribute',

			
			'addon_list'              => $vitepos_base . 'addon/list',
			'create_addon'            => $vitepos_base . 'addon/create',
			'update_addon'            => $vitepos_base . 'addon/update',
			'addon_details'           => $vitepos_base . 'addon/details',
			'delete_addon'            => $vitepos_base . 'addon/delete-addon',
			'change_addon_status'     => $vitepos_base . 'addon/change-status',
			
			'sync_order_list'         => $vitepos_base . 'restaurant/sync-order-list',
			'sync_order'              => $vitepos_base . 'restaurant/sync-order',
			'canned_message'          => $vitepos_base . 'restaurant/canned-message',
			
			'table_list'              => $vitepos_base . 'table/list',
			'create_table'            => $vitepos_base . 'table/create',
			'update_table'            => $vitepos_base . 'table/update',
			'table_details'           => $vitepos_base . 'table/details',
			'delete_table'            => $vitepos_base . 'table/delete-table',
			
			'send_to_kitchen'         => $vitepos_base . 'restaurant/send-to-kitchen',
			'waiter_order_list'       => $vitepos_base . 'restaurant/waiter-order-list',
			'resto_details'           => $vitepos_base . 'restaurant/details',
			'make_served'             => $vitepos_base . 'restaurant/make-served',
			'cancel_order'            => $vitepos_base . 'restaurant/cancel-order',
			'cancel_order_request'    => $vitepos_base . 'restaurant/cancel-order-request',
			'cancel_request_ans'      => $vitepos_base . 'restaurant/cancel-request-ans',
			'change_order_table'      => $vitepos_base . 'restaurant/change-table',
			
			'served_list'             => $vitepos_base . 'restaurant/served-list',
			'cashier_details'         => $vitepos_base . 'restaurant/cashier-details',
			'restaurant_payment'      => $vitepos_base . 'restaurant/restaurant-payment',
			
			'deny_item'               => $vitepos_base . 'restaurant/deny-item',
			'remove_item'             => $vitepos_base . 'restaurant/remove-item',
			'cancel_item_request'     => $vitepos_base . 'restaurant/cancel-item-request',
			'item_cancel_req_ans'     => $vitepos_base . 'restaurant/item-cancel-req-ans',
			'start_item'              => $vitepos_base . 'restaurant/start-item',
			'complete_item'           => $vitepos_base . 'restaurant/complete-item',
			'serve_item'              => $vitepos_base . 'restaurant/serve-item',
			'update_order'            => $vitepos_base . 'restaurant/update-order',
			
			'kitchen_order_list'      => $vitepos_base . 'restaurant/kitchen-order-list',
			'start_preparing'         => $vitepos_base . 'restaurant/start-preparing',
			'complete_preparing'      => $vitepos_base . 'restaurant/complete-preparing',
			'deny_order'              => $vitepos_base . 'restaurant/deny-order',
			'add_kitchen_note'        => $vitepos_base . 'restaurant/add-kitchen-note',
			
			'table_order_list'        => $vitepos_base . 'restaurant/table-order-list',
			'basic_remove_item'       => $vitepos_base . 'restaurant/basic-remove-item',
			
			'stock_transfer'          => $vitepos_base . 'purchase/stock-transfer',
			'stock_receive'           => $vitepos_base . 'purchase/stock-receive',
			'transfer_list'           => $vitepos_base . 'purchase/transfer-list',
			'transfer_details'        => $vitepos_base . 'purchase/transfer-details',
			'receive_list'            => $vitepos_base . 'purchase/receive-list',
			'stock_decline'           => $vitepos_base . 'purchase/stock-decline',
			'stock_accept'            => $vitepos_base . 'purchase/stock-accept',
			'stock_log'               => $vitepos_base . 'purchase/stock-log',
			'all_taxes'               => $vitepos_base . 'product/all-taxes',

			'product_stocks'          => $vitepos_base . 'purchase/product-stocks',

			
			'order_report'            => $vitepos_base . 'report/order-report',
			'product_report'          => $vitepos_base . 'report/product-report',
			'product_report_download' => $vitepos_base . 'report/product-report-download',
			'product_info'            => $vitepos_base . 'report/product-info',
			'product_details_report'  => $vitepos_base . 'report/product-details-report',
			'customer_report'         => $vitepos_base . 'report/customer-report',
			'staff_report'            => $vitepos_base . 'report/staff-report',
			'dashboard_report'        => $vitepos_base . 'report/dashboard-report',

			
			'tips_log'                => $vitepos_base . 'outlet/cash-drawer-tips',
			'user_tips_log'           => $vitepos_base . 'outlet/user-tips-log',
			'withdraw_tips'           => $vitepos_base . 'outlet/withdraw-tips',
		 ...
		 * @since 3.1.0
		 */
		$urls_3rd = apply_filters( 'vitepos/filter/api/urls', array(), $vitepos_base );
	...
	public function save_update_product_variation_img( $variation_id, $v_index, $product_id ) {
		...
		if ( ! empty( $files['variations']['name'][ $v_index ]['image'] ) ) {
			if ( empty( $files['variations']['error'][ $v_index ]['image'] ) ) {
				$attach_id = $this->insert_media_attachment(
					$files['variations']['tmp_name'][ $v_index ]['image'],
					$files['variations']['name'][ $v_index ]['image'],
					$files['variations']['type'][ $v_index ]['image'],
					$variation_id
				);
				if ( ! empty( $attach_id ) ) {
	...
	public function save_update_product_gallery_img( $product_id, $rm_gallery ) {
		...
		if ( $update_galleries != $image_galleries ) {
			update_post_meta( $product_id, '_product_image_gallery', $update_galleries ) || add_post_meta(
				$product_id,
	...
	public function save_update_product_feature_img( $product_id ) {
		...
		if ( ! empty( $files['feature_image']['name'] ) && empty( $files['feature_image']['error'] ) ) {
			$attach_id = $this->insert_media_attachment(
				$files['feature_image']['tmp_name'],
				$files['feature_image']['name'],
				$files['feature_image']['type'],
				$product_id
			);
			if ( ! empty( $attach_id ) ) {
	...
	/**
	 * The save update category img is generated by appsbd
	 *
	 * @param mixed $category_id It is category_id param.
	 */
	public function save_update_category_img( $category_id ) {
		$files = AppInput::get_uploaded_files();
		if ( ! empty( $files['category_image']['name'] ) && empty( $files['category_image']['error'] ) ) {
			$attach_id = $this->insert_media_attachment(
				$files['category_image']['tmp_name'],
				$files['category_image']['name'],
				$files['category_image']['type'],
				$category_id
			);
			if ( ! empty( $attach_id ) ) {
				update_term_meta( $category_id, 'thumbnail_id', $attach_id );
			}
		}
	}

	/**
	 * The save update user img is generated by appsbd
	 *
	 * @param mixed $user_id It is user_id param.
	 */
	public function save_update_user_img( $user_id ) {
		$files = AppInput::get_uploaded_files();
		if ( ! empty( $files['img']['name'] ) && empty( $files['img']['error'] ) ) {
			$attach_id = $this->insert_media_attachment(
				$files['img']['tmp_name'],
				$files['img']['name'],
				$files['img']['type'],
			...
			if ( ! empty( $attach_id ) ) {
				update_user_meta( $user_id, '_vtpos_user_img', $attach_id );
			}
		}
	}
	...
	public function on_init() {
		...
		if ( self::is_stockable() ) {
			if ( ! self::is_default_stock() ) {
				if ( self::is_stock_linked_with() ) { 
					...
					add_filter(
						'woocommerce_product_get_stock',
						array( $this, 'online_product_stock_quantity' ),
						999,
						3
					);
					add_filter(
						'woocommerce_product_variation_get_stock',
						array( $this, 'online_product_stock_quantity' ),
						999,
						3
					);
					add_filter(
						...
						array( $this, 'product_manage_stock' ),
						999,
						3
					);
					add_filter(
				...
				add_filter(
					'woocommerce_product_variation_is_in_stock',
					array( $this, 'online_product_in_stock' ),
					10,
					2
				);
			} else {
		...
		new POS_Order_Payment();
		new Tips_Log_Manage();
		

		
		
        if(vitepos_is_api_request() || vitepos_is_api_request('vitepos-user/v1/')) {
            add_filter( 'pre_option_woocommerce_feature_cost_of_goods_sold_enabled', array($this,'auto_set_cost_of_good'), 3, 99 );
        }
		add_filter('vitepos/filter/get-product-barcode',array($this,'get_product_barcode'));
        add_action("appsbd/vitepos/action/set-default-option",array($this,'set_default_pos_option'));

	}
	public function get_product_barcode($product) {
		$barcode= POS_Product::get_barcode_of_product($product);
		return $barcode;
	}

	/**
	 * The set default pos option is generated by appsbd
	 *
	 * @param mixed $data It is data param.
	 */
    public function set_default_pos_option($data){
        if(!empty($data)){
            if(!empty($data->pos_mode)){
                $this->options['pos_mode'] = $data->pos_mode;
            }
            if(!empty($data->is_kitchen)){
                $this->options['is_kitchen'] = $data->is_kitchen;
            }
            if(!empty($data->is_item_wise)){
                $this->options['is_item_wise'] = $data->is_item_wise;
            }
            if(!empty($data->is_basic_add_item)){
                $this->options['is_basic_add_item'] = $data->is_basic_add_item;
            }
            if(!empty($data->is_basic_remove_item)){
                $this->options['is_basic_remove_item'] = $data->is_basic_remove_item;
            }
            if(!empty($data->pusher)){
                $this->options['vtpos_push_setting'] = $data->pusher;
            }
            $this->update_option();
        }
    }

    public function auto_set_cost_of_good($pre, $option, $default_value) {
	    return 'yes';
    }
	/**
	...
	/**
	 * The get variation manage stock is generated by appsbd
	 *
	 * @param \WC_Product_Variation|\WC_Product $product It is variation param.
	 *
	 * @return int
	 */
    public static function get_manage_stock_without_parent($product) {
	    $manage_stock=0;
	...
	public function custom_product_status() {
		register_post_status(
			...
			array(
				...
				'label_count'               => _n_noop(
					'Vitepos Only <span class="count">(%s)</span>',
					'Vitepos Only <span class="count">(%s)</span>',
					'vitepos'
				),
			)
            ...
            <tr class="form-field">
                ...
                <td>
                    ...
                    <select name="outlets[]" id="user_select" multiple>
						...
						?>
                        <option <?php echo( empty( $selected_outlets ) ? esc_attr( 'selected' ) : '' ); ?>
                                value=""><?php $this->esc_e( 'None' ); ?></option>
						<?php
	...
	public function save_variation_barcode( $variation_id, $i ) {
		...
		update_post_meta( $variation_id, '_vt_barcode', esc_attr( $barcdoe ) ) || add_post_meta(
			$variation_id,
			'_vt_barcode',
			esc_attr( $barcdoe )
		);
	...
	public static function get_settings( $type = '' ) {
		 ...
		 * @since 3.0.4
		 */
		$basic_settings['is_incl_tax'] = apply_filters(
		...
		if ( isset( $basic_settings['pusher_o_sync_intval'] ) ) {
			unset( $basic_settings['pusher_o_sync_intval'] );
		}
		if ( isset( $basic_settings['pusher_p_sync_intval'] ) ) {
			unset( $basic_settings['pusher_p_sync_intval'] );
		}
        ...
        if(!empty($basic_settings['stockable']) && 'Y' == $basic_settings['stockable']){
            $basic_settings['hide_oos_product'] = $stock_settings->is_hide_out_stock;
        }
		if ( ! empty( $basic_settings['is_kitchen'] ) && 'Y' == $basic_settings['is_kitchen'] ) {
			$obj        = new \stdClass();
			$obj->title = 'Completed';
		...
		$basic_settings['fps'] = hash(
			'crc32b',
			$basic_settings['is_incl_tax'] . $basic_settings['stockable'] . $basic_settings['barcode_field'] . $basic_settings['pos_mode'] . $basic_settings['stock_type'] . wp_json_encode( $product_status )
		);
		if ( self::is_push_enabled() ) {
			$basic_settings['o_sync_intval'] = (int) self::get_module_option( 'pusher_o_sync_intval', 60000 );
			$basic_settings['p_sync_intval'] = (int) self::get_module_option( 'pusher_p_sync_intval', 600000 );
		} else {
			$basic_settings['o_sync_intval'] = (int) self::get_module_option( 'o_sync_intval', 30000 );
			$basic_settings['p_sync_intval'] = (int) self::get_module_option( 'p_sync_intval', 300000 );
		}
		 ...
		 * @since 3.1.0
		 */
		$settings->reward_settings = apply_filters( 'appsbd/vite-reward/filter/get-settings', null );
		/**
		 * Its for check is there any change before process
		 *
		 * @since 3.3.0
		...
		$settings = apply_filters('appsbd/vitepos/filter/settings',$settings);
		...
		$custom_fields = $custom_field->select_all_grid_data( '', 'fld_order', 'ASC' );
		foreach ( $custom_fields as &$field ) {
			if ( ! empty( $field->options ) && is_string( $field->options ) ) {
				$field->options = unserialize( $field->options );
			}
		}
		$settings->custom_fields     = apply_filters( 'appsbd/vitepos/filter/custom-fields', $custom_fields );
		...
		$settings->user_app_settings = apply_filters( 'appsbd/vitepos-user/get-user-app-settings', null );
		 ...
		 * @since 3.1.9
		 */
		$settings->addons = apply_filters( 'vitepos/settings/addons', array() );
	...
	public function get_admin_settings() {
		...
		if ( ! isset( $this->options['round_type'] ) ) {
			$this->options['round_type'] = 'S';
		}
		if ( ! isset( $this->options['customize_pricing'] ) ) {
			$this->options['customize_pricing'] = 'N';
		}
		if ( ! isset( $this->options['round_price'] ) ) {
			$this->options['round_price'] = 'N';
		}
        if ( ! isset( $this->options['drawer_counted_amount'] ) ) {
			$this->options['drawer_counted_amount'] = 'N';
		}
        if ( ! isset( $this->options['is_required_drawer_counted_amount'] ) ) {
			$this->options['is_required_drawer_counted_amount'] = 'N';
		}
		...
		if ( ! isset( $this->options['is_token_enabled'] ) ) {
			$this->options['is_token_enabled'] = 'N';
		}
		if ( ! isset( $this->options['enable_token_resetting'] ) ) {
			$this->options['enable_token_resetting'] = 'N';
		}
		if ( ! isset( $this->options['token_reset_time'] ) ) {
			$this->options['token_reset_time'] = '00:00';
		}
		if ( ! isset( $this->options['token_prefix'] ) ) {
		...
		$options['basic_settings']['p_sync_intval'] = $this->get_option( 'p_sync_intval', 300000 );
		$options['basic_settings']['o_sync_intval'] = $this->get_option( 'o_sync_intval', 30000 );

		$options['basic_settings']['pusher_p_sync_intval'] = $this->get_option( 'pusher_p_sync_intval', 600000 );
		$options['basic_settings']['pusher_o_sync_intval'] = $this->get_option( 'pusher_o_sync_intval', 60000 );
	...
	public static function get_stock_settings() {
		...
		if ( 'Y' == $stock_settings->is_stockable && empty( $stock_settings->is_hide_out_stock ) ) {
			$stock_settings->is_hide_out_stock = 'N';
		}
		if ( ! isset( $stock_settings->is_hide_out_stock ) ) {
			$stock_settings->is_hide_out_stock = 'N';
		}
	...
	public function refresh_app() {
		...
		if ( self::get_pos_mode() == 'R' || self::get_pos_mode() == 'B' ) {
		...
		if ( self::is_stock_linked_with() ) {
			...
			if ( $linked->linked_outlet == $outlet_id ) {
				...
				if ( Mapbd_pos_warehouse::transfer_online_to_outlet( $linked->linked_outlet, $stock_info ) ) {
					...
					$this->add_info(
						'Successfully Transferred,Product %s Stocks %s',
						$stock_info->products,
						$stock_info->stocks
					);
					$response->display_with_response( true );
	 ...
	 * @param mixed $with_point It is with_point param.
	 *
	 * @return \stdClass|null
	 */
	public static function get_customer_object_by_id( $id, $with_point = false ) {
		...
		if ( ! empty( $user ) ) {
			...
			if ( $with_point ) {
				 ...
				 * @since 3.0
				 */
				do_action( 'appsbd/reward/action/get-customer-data', $customer_obj );
			}
	...
	/**
	 * Returns a cache-busting version for static assets.
	 *
	...
	public function get_asset_version( $relative_path ) {
		...
		$abs_path      = dirname( $this->plugin_file ) . '/' . $relative_path;
		...
		return (string) $this->kernel_object->plugin_version;
	}
	...
	public function get_sw_link() {
		return $this->get_plugin_url( 'templates/pos-assets/service-worker.js?v=' . $this->get_asset_version( 'templates/pos-assets/service-worker.js' ) );
	...
	public function load_pos_client() {
        ...
        $poslink=apply_filters('appsbd/vitepos/pos-template',plugin_dir_path( $this->plugin_file ) . 'templates/pos.php');
		
		include_once $poslink;
	...
	public static function process_on_update() {
		...
		Mapbd_Pos_Tips_Log::create_db_table();
	...
	public function client_header_assets( $links ) {
		...
		$font_ver  = $this->get_asset_version( 'assets/font.css' );
		$color_ver = $this->get_asset_version( 'templates/pos-assets/css/color-' . $pos_color . '.css' );
		$css_ver   = $this->get_asset_version( 'templates/pos-assets/css/vitepos.css' );
		$js_ver    = $this->get_asset_version( 'templates/pos-assets/js/vitepos.js' );
		$links[] = array(
			'href' => $this->get_plugin_url( 'assets/font.css?v=' . $font_ver ),
			'rel'  => 'stylesheet',
		);
		$links[] = array(
			'href' => $this->get_plugin_url( 'templates/pos-assets/css/color-' . $pos_color . '.css?v=' . $color_ver ),
			'rel'  => 'stylesheet',
		);
		$links[] = array(
			'href' => $this->get_plugin_url( 'templates/pos-assets/css/vitepos.css?v=' . $css_ver ),
			'rel'  => 'preload',
			'as'   => 'style',
		);
		$links[] = array(
			'href' => $this->get_plugin_url( 'templates/pos-assets/js/vitepos.js?v=' . $js_ver ),
			'rel'  => 'preload',
			'as'   => 'script',
		);
		$links[] = array(
	...
	public function client_footer_scripts( $scripts ) {
		...
		$scripts[] = array(
			'src' => $this->get_plugin_url( 'templates/pos-assets/js/vitepos.js?v=' . $this->get_asset_version( 'templates/pos-assets/js/vitepos.js' ) ),
		);
	...
	/**
	 * The get assigned waiters is generated by appsbd
	 *
	 * @param mixed $table_id It is table_id param.
	 *
	 * @return array|string[]
	 */
	public function get_assigned_waiters( $table_id ) {
		$waiters = array();
		$table   = new Mapbd_Pos_Table();
		$table->id( $table_id );
		if ( $table->select( 'id,assigned_waiters' ) ) {
			$waiters = explode( '|', $table->assigned_waiters );
		}
	...
	public function discount_total_line( $order ) {
		if ( $order->get_total_fees() < 0 ) {
            ...
            <tr>
                <td class="label">
                    ...
                    <small><i><?php empty( $coupons ) ? $this->esc_e( '(Fee - Discount)' ) : $title; ?></i></small>:
                </td>
                <td width="1%"></td>
                <td class="total">
					<?php
					...
					if ( empty( $coupons ) ) {
						echo $discount > 0 ? esc_html( $discount ) : '';
					}
					echo wp_kses_post(
						wc_price(
							$fee,
		...
		if ( 0.0 != $rounding ) {
            ...
            <tr>
                ...
                <td class="total">
					<?php
					echo wp_kses_post( wc_price( $rounding ) );
					?>
                </td>
            </tr>
			<?php
	...
	public function show_order_meta( $order_id ) {
		...
		if ( $is_vite_post ) {
			?>
            <tr>
                <td colspan="3" class="label label-highlight"><i
                            class="vps vps-vt-pos vtp-order-dtls-icon"></i> <?php $this->esc_e( 'Information' ); ?></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
					...
					if ( ! empty( $counter_id ) ) {
						$counter_info = Mapbd_pos_counter::find_by(
							'id',
							$counter_id,
	...
	public function send_customer_email( $order_id ) {
		...
		if ( $is_need_to_send ) {
			...
			if ( isset( $email_list['WC_Email_Customer_Invoice'] ) ) {
				$email_list['WC_Email_Customer_Invoice']->trigger( $order_id );

				$email_count = $order->get_meta( '_vtp_send_em_count', true );
				$order->update_meta_data( '_vtp_send_em_count', $email_count ? $email_count + 1 : 1 );
	 ...
	 * @return bool
	 */
	public static function is_tax_qr_enabled() {
		$inv_settings = Invoice_Settings::get_settings();
		return $inv_settings->show_tax_qr_code;
	}
    /**
	 * The is push enabled is generated by appsbd
	 *
	 * @return bool
	 */
	public static function is_tax_qr_position() {
		$inv_settings = Invoice_Settings::get_settings();
		return $inv_settings->tax_qr_code_position;
	}
