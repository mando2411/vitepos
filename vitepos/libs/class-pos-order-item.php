...
class POS_Order_Item {
	 ...
	 */
	public static function get_product_item( $item, &$order, &$v_order = null ) {
		...
		$o_item->category_ids = $item->get_meta( '_vtp_category_ids' );
		...
		foreach ( $order->get_taxes() as $tax_item ) {
			...
			$tax->amount = 0.0;
			$tax->percentage = $tax_item->get_rate_percent();
			$tax_list[ $tax->id ] = $tax;
		}
		...
		self::set_item_wise_fee_discount( $o_item, $item, $order, $v_order->v_total_fees, $v_order->v_total_discount );
		...
		if ($order->meta_exists('_vtp_up_batch') ) {
			if (!empty($item->get_meta('_vtp_it_batch')))
			{
				$o_item->item_batch = $item->get_meta('_vtp_it_batch');
			}else{
				$o_item->item_batch=0;
			}
		}
	 ...
	 */
	public static function set_item_wise_fee_discount( &$resp_item, $item, $order, $total_fee, $total_dis ) {
		...
		if ( true || empty( $codes ) ) {
			...
			if ( 0 < $order_total && ( 0 < $total_dis || 0 < $total_fee ) ) {
				if ( $include_tax && $item->meta_exists( '_vtp_items_price' ) ) {
					$item_sub_total = (float) $item->get_meta( '_vtp_items_price' );
				} else {
					$item_sub_total = (float) $item->get_subtotal();
				}
				if ( $include_tax && $resp_item->addon_total > 0 ) {
					$item_sub_total += $resp_item->addon_total;
				}
				if ( $total_dis > 0 ) {
			...
			$resp_item->discount = floatval( $item->get_subtotal() - $item->get_total() );
			$resp_item->discount_amount = 0 != $resp_item->discount ? floatval( $resp_item->discount / $resp_item->quantity ) : 0;
		}
	}
