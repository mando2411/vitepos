...
class POS_Payment {
	...
	public function __construct( $data, $outlet_id, $counter_id ) {
		...
		do_action( 'vitepos/action/adjust-decimal' );
	}
	...
	private function create_order() {
		...
		if ( ! empty( $this->payload['items'] ) ) {
			...
			if ( POS_Settings::get_is_order_token() ) {
				$this->set_order_token_no();
			}
			$total_tax = 0.0;
			...
			foreach ( $this->payload['items'] as $item ) {
				...
				try {
					...
					if ( ! empty( $item['category_ids'] ) ) {
						$oitem->add_meta_data( '_vtp_category_ids', $item['category_ids'] );
					}
					 ...
					 */
					do_action_ref_array(
						'appsbd/vitepos/action/order/line-item',
						array( &$oitem, &$item, &$this->order, $coupons )
					);
					$oitem->save();
			...
			if ( $coupon_total > 0 ) {
				 ...
				 * @since 3.0.4
				 */
				$is_inclusive_enabled = apply_filters(
				...
				if ( $order_total != $grand_total ) {
					try {
						...
						if ( $miss > 0 && $is_inclusive_enabled ) {
			...
			if ( $this->get_payload( 'custom_fields', '' ) != '' ) {
				if ( ! Mapbd_Pos_Custom_Field::validate_custom_fields(
					$this->get_payload( 'custom_fields', array() ),
					'I'
				) ) {
					return false;
				} else {
			...
			if ( $this->is_offline ) {
				...
				$processed_by_offline_date = $this->get_payload( 'offline_order_time' );
				if (!empty($processed_by_offline_date))
				{
					$this->add_meta_data( '_vtp_offline_process_date', $processed_by_offline_date );
				}
			}
	...
	/**
	 * The set order token no is generated by appsbd
	 */
	protected function set_order_token_no() {
		$token_no = POS_Settings::get_token_no();
		if ( ! empty( POS_Settings::get_token_prefix() ) ) {
			$token_no = POS_Settings::get_token_prefix() . '' . $token_no;
		}
		$this->order->update_meta_data( '_vtp_token_no', $token_no );
	}
	...
	protected function check_items_stock() {
		...
		foreach ( $this->payload['items'] as $item ) {
			...
			if ( POS_Settings::get_manage_stock_without_parent($product) && $current_stock_float < $qty ) {
				$is_ok                  = false;
	 ...
	 * @return float|int
	 */
	protected function get_total_custom_fee_or_discount( $subtotal, $fee_or_discounts ) {
		$amount = 0.00;
		if ( ( ! empty( $fee_or_discounts ) && is_array( $fee_or_discounts ) ) ) {
			foreach ( $fee_or_discounts as $item ) {
				if ( strtoupper( $item['is_taxable'] ) == 'Y' ) {
					if ( strtoupper( $item['amount_type'] ) == 'P' ) {
						$amount = $amount + ( floatval( $subtotal ) ) * ( floatval( $item['val'] / 100 ) );
					} else {
						$amount = $amount + ( floatval( $item['val'] ) );
					}
	...
	protected function set_fee_discount() {
		...
		if ( $is_inclusive_enabled && 'B' == $tax_method ) {
			$this->calculate_totals();
			$total_amount = $this->order->get_total();
		} else {
			$total_amount = $this->order->get_subtotal();
			if ( $is_inclusive_enabled && 'A' == $tax_method ) {
				$final_sub_total = $total_amount + $this->order->get_total_tax();
				$this->add_meta_data( '_vt_sub_total', $final_sub_total );
			}
		...
		if ( ! empty( $this->payload['c_fees'] ) && is_array( $this->payload['c_fees'] ) ) {
			foreach ( $this->payload['c_fees'] as $item ) {
				if ( ! empty( $item['amount_type'] ) && ! empty( $item['val'] ) ) {
					$item_val = floatval( $item['val'] );
					$title    = POS_Settings::get_module_instance()->__( $item['title'] );
					if ( $item_val > 0 ) {
						if ( strtoupper( $item['amount_type'] ) == 'P' ) {
							$item_amount = $total_amount * ( $item_val / 100 );
							$title       .= '(' . $item['val'] . '%)';
						...
						vitepos_order_add_fee_on_order(
							...
							$item_amount,
							array(
								'_vtp_cal_type'    => $item['amount_type'],
								'_vtp_cal_val'     => $item['val'],
								'_vtp_cal_amount'  => $item['amount'],
								'_vtp_custom_type' => $item['type'],
								'_vtp_title'       => $item['title'],
								'_vtp_is_taxable'  => $item['is_taxable'],
								'_vtp_fee_uid'     => $item['uid'],
							)
						);
					}
				}
		...
		if ( ! empty( $this->payload['c_discounts'] ) && is_array( $this->payload['c_discounts'] ) ) {
			foreach ( $this->payload['c_discounts'] as $item ) {
				if ( ! empty( $item['amount_type'] ) && ! empty( $item['val'] ) ) {
					$item_val = floatval( $item['val'] );
					$title    = POS_Settings::get_module_instance()->__( $item['title'] );
					if ( $item_val > 0 ) {
		 ...
		 * @since 3.1.6
		 */
		do_action_ref_array( 'appsbd/vitepos/add-custom-fee-discount', array( &$this->order, &$this->payload ) );
		$tax_method = $this->order->get_meta( '_vt_tax_method' );
	...
	protected function set_tax_after_discount_or_fee() {
		...
		if ( 'A' == $tax_method ) {
			...
			if ( ! empty( $this->payload['c_fees'] ) && is_array( $this->payload['c_fees'] ) ) {
				$fee_total += $this->get_total_custom_fee_or_discount( $total_amount, $this->payload['c_fees'] );
			}
			if ( ! empty( $this->payload['c_discounts'] ) && is_array( $this->payload['c_discounts'] ) ) {
				$discount_total += $this->get_total_custom_fee_or_discount(
					$total_amount,
					$this->payload['c_discounts']
				);
			}
	...
	protected function is_ready_to_checkout() {
		 ...
		 * @since 4.0.0
		 */
		$is_reward_ok = apply_filters(
			'appsbd/rwd/filter/is-ok-reward',
			array(
				'is_ok' => true,
				'msg'   => '',
			),
		...
		if ( ! $is_reward_ok['is_ok'] ) {
			POS_Settings::get_module_instance()->add_error( $is_reward_ok['msg'] );
		...
		$order_fees  = (float) $this->order->get_total_fees();
		/**
		 * Its for check is the tax option is inclusive or exclusive.
		 *
		 * @since 3.0.4
			...
			try {
				if ( ! $is_inclusive_enabled ) {
					...
					if ( $miss < 0.2 && $miss > ( - 0.2 ) ) {
						if ( 'A' == $tax_method ) {
							if ( intval( $miss * 1000 ) == intval( $order_fees * 1000 ) ) {
								$this->add_meta_data( '_vtp_miss_total', 0.0 );
							} else {
								$this->add_meta_data( '_vtp_miss_total', ( - 1 ) * $miss );
							}
	...
	public function send_to_kitchen( $is_offline = false ) {
		...
		$waiter_id           = $this->get_payload( 'waiter_id', null );
		if ( ! empty( $waiter_id ) ) {
			$get_outlets = get_user_meta( $waiter_id, 'outlet_id', true );
			if ( ! empty( $get_outlets ) ) {
				if ( ! in_array( $this->outlet_id, $get_outlets ) ) {
					POS_Settings::get_module_instance()->add_error( 'You have no permission to create order' );
		...
		if ( $this->create_order() ) {
			...
			$waiter_id = $this->get_payload( 'waiter_id', null );
			if ( ! empty( $waiter_id ) ) {
				$this->add_meta_data( '_vtp_order_by', $waiter_id );
			} else {
			...
			$this->set_fee_discount();

			$grand_total = $this->get_payload( 'grand_total' );
			if ( $this->order->get_total() != $grand_total ) {
				$this->order->set_total( $grand_total );
				$this->order->save();
			}
			if ( 'R' == POS_Settings::get_pos_mode() ) {
				$order_status = $this->order->update_status( 'vt_in_kitchen', 'Sent to kitchen', true );
				$this->add_time_by_status( 'vt_in_kitchen' );
				$this->add_meta_data( '_vt_is_paid', 'N' );
				POS_Settings::get_module_instance()->add_info( 'Order successfully sent to kitchen' );
			} else {
				if ( 'B' == POS_Settings::get_pos_mode() && ! $is_offline ) {
					$order_status = $this->order->update_status( 'vt_processing', 'In Process', true );
					$this->add_time_by_status( 'vt_processing' );
					$this->add_meta_data( '_vt_is_paid', 'N' );
					POS_Settings::get_module_instance()->add_info( 'Order is in processing' );
				} elseif ( ! empty( $this->payload['status'] ) ) {
					$order_status = $this->order->update_status( 'vt_processing', 'In Process', true );
					$this->add_time_by_status( 'vt_processing' );
					$this->add_meta_data( '_vt_is_paid', 'N' );
					if ( $this->payload['status'] == 'vt_processing' ) {
						POS_Settings::get_module_instance()->add_info( 'Order is in processing' );
					} else {
						$this->restaurant_checkout( $this->order->get_id(), $is_offline );
	 ...
	 */
	public function restaurant_checkout( $order_id, $is_offline = false ) {
		$this->is_restaurant = true;
		$this->is_checkout   = false;
		$this->is_offline    = $is_offline;
		...
		if ( $is_offline ) {
			if ( ! $this->order->meta_exists( '_vtp_offline_id' ) ) {
				vitepos_wc_update_meta( $order_id, '_vtp_offline_id', $this->payload['offline_id'] );
			}
			$processed_by_offline_date = $this->get_payload( 'offline_order_time' );
			if (!empty($processed_by_offline_date))
			{
				vitepos_wc_update_meta( $order_id, '_vtp_offline_process_date', $processed_by_offline_date );
			}

		}

		if ( ! empty( $this->order ) && ( 'vt_served' == $stat || 'pending' == $stat || 'failed' == $stat || 'vt_ready_to_srv' == $stat || 'vt_processing' == $stat ) ) {
			...
			if ( $this->is_ready_to_checkout() ) {
				...
				return $this->_complete_order( $is_offline );
			}
		} else {
	...
	public function order_checkout( $order_id ) {
		...
		if ( ! empty( $this->order ) && ( 'vtu_ready_to_pick' == $stat || 'vt_ready_to_srv' == $stat || 'vt_preparing' == $stat ) ) {
			vitepos_wc_update_meta( $order_id, '_vtp_prev_status', $stat );
			$billing_address = $this->get_outlet_address();
			$customer_id     = $this->order->get_customer_id();
			if ( empty( $customer_id ) ) {
	...
	public function cancel_order() {
		...
		if ( ! empty( $this->order ) ) {
			if ( 'pending' == $status || 'failed' == $status ) {
	...
	/**
	 * The load order from payload is generated by appsbd
	 */
	...
	public function complete_order_payment() {
		...
		if ( ! empty( $this->order ) ) {
			if ( 'pending' == $status || 'processing' == $status || 'on-hold' == $status ) {
				 ...
				 */
				do_action_ref_array(
					"vitepos/action/payment-method/complete-order/{$id}",
					array( &$this, &$paymentlist, &$payment_status_ok )
				);
				...
				return $this->_complete_order();
			} elseif ( 'complete' == $status || 'completed' == $status ) {
				if ( isset($paymentlist[0]['type']) && $paymentlist[0]['type'] == 'B')
				{
					/**
					 * Its for order complete
					 *
	...
	protected function update_cash_drawer() {
		...
		if ( ! $is_found_cash_drawer ) {
			if ( ! $this->is_offline ) {
				...
				if ( $this->order->meta_exists( '_vtp_counter_id' ) ) {
					if ($this->order->get_meta( '_vtp_counter_id' )!='undefined')
					{
	...
	protected function process_payment( &$payment_item ) {
		 ...
		 * @since 2.4
		 */
		$filter_item = apply_filters(
			"vitepos/filter/payment/process/{$type}",
			$filter_item,
			$currency_code,
			$this->order,
			$this->outlet_id,
	...
	protected function _complete_order( $is_offline = false, $can_delete = false ) {
		...
		if ( ! $is_offline ) {
			...
			if ( $payment_response->order_status ) {
				$status = 'completed';
				if ( ! $this->order->meta_exists( '_is_vtp_user' ) ) {
				...
				if ( $this->order->update_status( $status, 'Vitepos checkout', true ) ) {
					...
					if ( ! $payment_response->need_payment ) {
						if ( $is_restaurant || $this->is_restaurant ) {
							if ( $is_pay_first && POS_Settings::is_kitchen() && ! $this->order->meta_exists( '_is_vtp_user' ) ) {
								POS_Settings::get_module_instance()->add_info( 'Order successfully sent to kitchen' );
							} else {
								POS_Settings::get_module_instance()->add_info( 'Order successful' );
							}
					...
					if ( $is_complete ) {
						...
						if ( POS_Settings::is_enable_cashier_email() || POS_Settings::is_enable_customer_email() || ( empty( $this->outlet_obj->email ) || $this->outlet_obj->email != $this->order->get_billing_email( '' ) ) ) {
					...
					$points = apply_filters(
						'appsbd/reward/action/get-customer-current_point',
						$this->order->get_customer_id()
					);
					if ( appsbd_is_activated_plugin( 'vite-rewards-pro/vite-rewards-pro.php' ) && ! empty( $points ) && $points > 0 ) {
						vitepos_wc_update_meta( $this->order->get_id(), '_vtp_current_points', $points );
					}
					$order_details = POS_Order::get_from_woo_order_restro_by_id(
						$this->order->get_id(),
						false,
	...
	/**
	 * The hook order complete is generated by appsbd
	 */
	public function hook_order_complete() {
		/**
		 * Its for check is there any change before process
		 *
		 * @since 3.1.6
		 */
		do_action_ref_array(
			'appsbd/vitepos/action/vt-order-complete',
			array( &$this->order )
		);
	}
	...
	protected function get_order_complete_status( $is_restaurant, $is_pay_first ) {
		...
		if ( $is_restaurant ) {
			if ( $is_pay_first ) {
				if ( POS_Settings::is_kitchen() && 'Y' != $this->order->is_user ) {
