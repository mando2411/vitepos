...
use WPForms\Vendor\Stripe\ThinEvent;
...
class POS_Product {
	...
	public static function set_purchase_cost($product_id,$val=0.0) {
		if (empty($product_id))
		{
			return false;
		}
		return update_post_meta($product_id,'_cogs_total_value', $val);
	}
	/**
	 * The get product data is generated by appsbd
	 *
	 * @param \WC_Product|\WC_Product_Variation|\WC_Product_Simple   $product Its string.
	...
	public static function get_product_data( $product, $add_ctg_id = false, $is_no_variable = false ) {
		...
		$pos_product->is_virtual        = $product->get_virtual();
		...
		$pos_product->sale_price = static::get_product_inclusive_price( $sale_price, $product );
		...
		self::get_product_category_ids_with_parents( $pos_product->id, $pos_product->category_ids, $pos_product->categories, $add_ctg_id );
		...
		$pos_product->purchase_cost  = get_post_meta($product->get_id(),'_cogs_total_value',true);
		if(empty($pos_product->purchase_cost)){
			$pos_product->purchase_cost=0;
		}
	...
	/**
	 * The get product category ids with parents is generated by appsbd
	 *
	 * @param int|mixed $product_id It is product_id param.
	 * @param array     $categories_ids It is categories_ids param.
	 * @param array     $categories It is categories param.
	 * @param boolean   $add_ctg_id It is add_ctg_id param.
	 */
	public static function get_product_category_ids_with_parents( $product_id, &$categories_ids = array(), &$categories = array(), $add_ctg_id = false ) {
		$category_list = wp_get_post_terms( $product_id, 'product_cat', array( 'fields' => 'all' ) );
		...
		if ( ! empty( $category_list ) && ! is_wp_error( $category_list ) ) {
			foreach ( $category_list as $category ) {
				...
				$category_ids[] = $current_category->term_id;

				$layer = 0;
				while ( $current_category && $layer < 3 ) {
					$category_ids[] = $current_category->term_id;
					if ( $add_ctg_id ) {
		...
		$categories_ids = array_values( array_unique( $category_ids ) );
	}
	...
	public static function get_product_variation_data( $product ) {
		...
		$pos_product->type          = $product->get_type();
		$pos_product->name          = strip_tags( $product->get_name(), '<strong><em>' ); 
		$pos_product->image         = self::get_wc_product_image( $product, 'woocommerce_thumbnail' );
	 ...
	 *
	 * @param \WC_Product_Variation $product Its string.
	 *
	 * @return array
	 */
	private static function get_variation_data( $product ) {
		...
		foreach ( $product->get_children() as $child_id ) {
			...
			$regular_price                = $variation->get_regular_price() ? $variation->get_regular_price() : 0;
			$price                        = $variation->get_price() ? $variation->get_price() : 0;
			$sale_price                   = $variation->get_sale_price() ? $variation->get_sale_price() : 0;
			$variation_obj->price         = static::get_product_inclusive_price( $price, $variation );
			$variation_obj->regular_price = static::get_product_inclusive_price( $regular_price, $variation );
			$variation_obj->sale_price    = static::get_product_inclusive_price( $sale_price, $variation );
			...
			if ( POS_Settings::is_default_stock() ) {
				 ...
				 * @since 1.0.0
				 */
				$variation_obj->stock_quantity = apply_filters(
			...
			$variation_obj->tax_rate            = NumberUtil::round(
				( doubleval( wc_get_price_including_tax( $variation ) ) - doubleval( $variation_obj->price ) ),
				wc_get_price_decimals()
			);
			$variation_obj->tax_rates = [];
			...
			if ( method_exists( $variation, 'get_global_unique_id' ) ) {
				$variation_obj->global_unique_id = $variation->get_global_unique_id();
			} else {
				$variation_obj->global_unique_id = $variation->get_sku() ? $variation->get_sku() : $variation->get_id();
			}

			
			$vari_cost = get_post_meta($variation->get_id(),'_cogs_total_value',true);
			$variation_obj->purchase_cost       = !empty($vari_cost) ? $vari_cost : 0.0;
	...
	/**
	 * Delete a variation product permanently
	 *
	 * @param int $child_id Variation ID
	 */
	public static function delete_variation_product( $child_id ) {
		...
		if ( $variation ) {
			...
			delete_post_meta( $child_id, '_sku' );
