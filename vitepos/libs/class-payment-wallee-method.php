 ...
 * @package VitePos\Libs
 */
class Payment_Wallee_Method extends Payment_Method_Base {
	/**
	...
	public function initialize() {
		...
		add_filter( 'vitepos-user/filter/header-links', array( $this, 'prefetch_script' ), 11 );
		add_filter( 'vitepos/filter/footer-scripts', array( $this, 'footer_script' ), 9 );
		add_filter( 'vitepos-user/filter/footer-scripts', array( $this, 'footer_script' ), 9 );
	...
	public function load_data() {
		...
		if ( isset( $settings['w_secret_key'] ) ) {
			$this->secret_key = $settings['w_secret_key'];
			unset( $settings['w_secret_key'] );
		}
	 ...
	 * @param \WC_Order  $order Its order param.
	 * @param int|string $outlet_id Its order param.
	 * @param int|string $counter_id Its order param.
	 *
	 * @return mixed
	 */
	...
	public function payment_capture( $order ) {
		...
		foreach ( $payment_list as &$payment_item ) {
			if ( $this->id == $payment_item['type'] ) {
				if ( ! empty( $payment_item['req_cap'] ) && 'Y' == $payment_item['req_cap'] && ! empty( $payment_item['intend_id'] ) ) {
	...
	/**
	 * The get stripe settings is generated by appsbd
	 *
	...
	/**
	 * The get intend status is generated by appsbd
	 *
	 * @param mixed $intend_id It is intend_id param.
	 * @param mixed $stripe_settings It is stripe_settings param.
	 *
	 * @return string|null
	 */
	...
	/**
	 * The set stripe api error msg is generated by appsbd
	 *
	 * @param mixed $e It is e param.
	 * @param mixed $stripe It is stripe param.
	 * @param mixed $intend_id It is intend_id param.
	 */
	public function set_stripe_api_error_msg( $e, $stripe, $intend_id ) {
		POS_Settings::get_module_instance()->add_error( $e->getMessage() );
	}
	/**
	...
	public function set_admin_settings() {
		...
		$this->set_tab_title( 'Wallee Settings', 'vps vps-cc-stripe' );

		$stripe_settings_card = $this->add_admin_card( 'settings', 'Wallee' );
		$stripe_settings_card->add_settings_field( 'Space Id', 'w_space_id', true, 'T' );
		$stripe_settings_card->add_settings_field( 'User Id', 'w_user_id', true, 'T' );
		$stripe_settings_card->add_settings_field( 'Secret Key', 'w_secret_key', true, 'E' );
