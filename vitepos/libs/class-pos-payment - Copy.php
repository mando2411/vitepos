...
class POS_Payment {
	...
	protected function set_fee_discount() {
		...
		$this->set_tax_after_discount_or_fee($total_amount,$is_inclusive_enabled);
	 ...
	 */
	protected function set_tax_after_discount_or_fee($total_amount=null,$is_inclusive_enabled=false) {
		$tax_method = $this->order->get_meta( '_vt_tax_method' );
		if ( 'A' == $tax_method ) {
	...
	protected function is_ready_to_checkout() {
		 ...
		 * @since 4.0.0
		 */
		$is_reward_ok = apply_filters( 'appsbd/rwd/filter/is-ok-reward', [ 'is_ok' => true, 'msg' => '' ], $this->order,
			$this->payload );
