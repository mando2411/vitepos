			...
			foreach ( $this->src_props as $src_prop ) {
				...
				if ( '*' == $src_prop['prop'] ) {
					...
					AND(({$this->wp_post}.post_title LIKE '%{$prop_val}%') 
						...
						OR ({$this->wp_post_meta}.meta_key = '_global_unique_id' AND {$this->wp_post_meta}.meta_value LIKE '%{$prop_val}%' ) 
		...
		if ( ! empty( $this->sort_props ) ) {
			foreach ( $this->sort_props as $prop ) {
				if ( 'is_favorite' == $prop['prop'] ) {
					...
					$this->set_join_table( 'mtp', '_price', 'INNER' );
					$this->order_by = 'mtp.meta_value ' . $prop['ord'];
				} elseif ( 'stock_quantity' == $prop['prop'] ) {
					$this->set_join_table( 'mtp', '_stock', 'INNER' );
					$this->order_by = 'mtp.meta_value ' . $prop['ord'];
