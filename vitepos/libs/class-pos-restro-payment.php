...
class POS_Restro_Payment extends POS_Payment {
	...
	public function remove_item( $order_id, $item_id ) {
		...
		if ( 'R' == POS_Settings::get_pos_mode() ) {
			$item_status = array( 'vt_it_kitchen', 'vt_it_denied', 'vt_it_accept_req', 'vt_it_placed', 'vt_it_picked' );
		}
		if ( 'B' == POS_Settings::get_pos_mode() ) {
			$item_status = array( '' );
		}
		if ( ! empty( $item ) && in_array( $item->get_meta( '_vtp_item_status' ), $item_status ) ) {
	...
	public function add_item( $order_id, $items, &$is_item_added ) {
		...
		if ( ! empty( $items ) ) {
			...
			foreach ( $items as $item ) {
				...
				try {
					...
					if ( ! empty( $item['variation_id'] ) ) {
						...
						if ( ! empty( $item['addon_total'] ) && ! empty( $item['addon_total'] ) ) {
							$item_regular_price = floatval( $product->get_regular_price( '' ) );
							$item_sale_price    = floatval( $product->get_sale_price( '' ) );
							$item_price         = floatval( $product->get_price( '' ) );
							$price              = $item_price + floatval( $item['addon_total'] );
					...
					if ('B' == POS_Settings::get_pos_mode())
					{
						$ord_batch = $this->order->get_meta('_vtp_up_batch');
						if (empty($ord_batch))
						{
							$ord_batch=1;
							$oitem->add_meta_data('_vtp_it_batch',$ord_batch);
						}else{
							$ord_batch = floatval($ord_batch) +1;
							$oitem->add_meta_data('_vtp_it_batch',floatval($ord_batch));
						}
					}
			...
			if ('B' == POS_Settings::get_pos_mode()) {
				if ( floatval( $ord_batch ) > 0 ) {
					if ($this->order->meta_exists('_vtp_up_batch'))
					{
						$this->order->update_meta_data('_vtp_up_batch', floatval( $ord_batch ));
					}else{
						$this->order->add_meta_data( '_vtp_up_batch', floatval( $ord_batch ) );
					}
					$this->order->save();
				}
			}
