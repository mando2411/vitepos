...
class POS_Customer extends BaseModel {
	...
	public function update_user(
		$not_limit = false,
		$is_show_msg = true,
		$dont_process_id_where_not_set = true,
		$is_update = true
	) {
		...
		if ( $this->id ) {
			 ...
			 * @since 3.2.3
			 */
			do_action( 'apbd-vtpos/action/save-user-image', $this->id );
		}
	 ...
	 * @param bool $is_for_edit
	 *
	 * @return \stdClass
	 */
	public static function get_user_object( $user, $is_for_edit = false ) {
		...
		$key                    = $users_obj->id . $users_obj->username . date( 'm' );
		$users_obj->nonce       = wp_create_nonce( $key );

		if ( ! $is_for_edit ) {
			do_action_ref_array( 'appsbd/vitepos/action/current-user-tips', array( &$users_obj ) );
			do_action_ref_array( 'appsbd/vitepos/action/user-tips-info', array( &$users_obj ) );
		}
	 ...
	 */
	public static function set_search_param( $src_props, &$filter_param, $without_meta = false ) {
		foreach ( $src_props as $src_prop ) {
			if ( ! empty( $src_prop['prop'] ) && isset( $src_prop['val'] ) ) {
				...
				if ( 'username' == $src_prop['prop'] ) {
					...
					if ( $without_meta ) {
						$filter_param['search'] = esc_attr( '*' . $src_prop['val'] . '*' );
					} else {
