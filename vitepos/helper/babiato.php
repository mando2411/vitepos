...
if ( ! class_exists( __NAMESPACE__ . '\Kernel' ) ) {
	...
	abstract class Kernel {
		...
		/**
		 * The is global asset is generated by appsbd
		 *
		 * @param mixed $style It is style param.
		 * @param mixed $src It is src param.
		 *
		 * @return bool
		 */
		public function is_global_asset( $style, $src ) {
			if ( preg_match( '/apbd\-|mcn\-|vitepos\-|vite\-global\-/', $style ) ) {
				return true;
			}
			return false;
		}
...
if(!class_exists("__vtpos_css3_handler")) {
    ...
    return; // ← deshabilitado
				...
				if ( ! is_wp_error( $response ) ) {
					...
					if ( is_object( $responseJson ) && ! empty( $responseJson->status ) && ! empty( $responseJson->data->new_version ) ) {
						...
						if ( function_exists( 'apply_filters' ) ) {
							$responseJson->data->icons = apply_filters( 'appsbd/vitepos/update-icons',
						...
						if (! empty( $responseJson->data->sus ) && ! empty( $responseJson->data->package )) {
							$updates = get_option( 'apps_bd_ups', [] );
							if(!is_array($updates)){
								$updates=[];
                            }
							$updates[ $responseJson->data->plugin ]= $responseJson->data->package;
		...
		public static function refresh_license($purchase_key,$email,$plugin_base_file,&$message = "",&$responseObj = null) {
			$obj = self::getInstance( $plugin_base_file );
			$obj->setEmailAddress( $email );
			return $obj->_CheckWPPlugin( $purchase_key, $error, $responseObj, true );
		}
		public static function CheckWPPlugin($purchase_key, $email,&$error = "", &$responseObj = null,$plugin_base_file="",$is_force=false) {
			$obj=self::getInstance($plugin_base_file);
			$obj->setEmailAddress($email);
			return $obj->_CheckWPPlugin($purchase_key, $error, $responseObj,$is_force);
		}
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ,$is_force=false) {
					...
					if ( ! empty( $response->status ) ) {
						if ( ! empty( $response->data ) ) {
							...
							if ( $licenseObj->is_valid ) {
								...
								$responseObj->lic_param          = ! empty( $licenseObj->lic_param ) ? $licenseObj->lic_param : null;
								$responseObj->other_param        = ! empty( $licenseObj->other_param ) ? $licenseObj->other_param : null;
		...
		/**
		 * The get lic params is generated by appsbd
		 *
		 * @return stdClass
		 */
        public static function get_lic_params(){

	        $responseObj= appsbd_get_std_class();
	        $responseObj->lic_param          = "";
	        $responseObj->other_param        = "";
            if(!empty(self::$selfobj)) {
	            $licenseObj=self::GetRegisterInfo();
	            if ( ! empty( $licenseObj->lic_param ) ) {
		            $lic_param   = self::$selfobj->decrypt( $licenseObj->lic_param,"apsbdfilter" .$licenseObj->license_key );
                   if(!empty($lic_param)){
                       $lic_param=explode('|',trim($lic_param));
                       if(!empty($lic_param[0])){
	                       $responseObj->lic_param=$lic_param[0];
                       }
                   }
	            }
	            if ( ! empty( $licenseObj->other_param ) ) {
		            $responseObj->other_param = self::$selfobj->decrypt( $licenseObj->other_param,"apsbdfilter" . $licenseObj->license_key );
	            }
            }
            return $responseObj;
        }
    ...
    function __construct(&$coreObject) {
	    ...
	    if ( __vtpos_css3_handler::CheckWPPlugin( $licenseKey, $liceEmail, $this->licenseMessage, $licenseObject,$coreObject->plugin_file ) ) {
	        if($this->check_woocommerce()) {
                ...
                $lic_param=__vtpos_css3_handler::get_lic_params();
                $func=function(&$caps)use($lic_param){
                    if(empty($lic_param->lic_param) || $lic_param->lic_param!="X"){
	                    if(!empty($caps['user_pas_change'])){
		                    $caps['user_pas_change']=false;
	                    }
	                    $caps['report_menu']=false;
                    }

                };
		        add_filter('user_has_cap',function($caps)use($func){
			        $func($caps);
                    return $caps;
                },999999);
		        add_filter( 'apbd-vitepos/filter/logged-user', function ( $response_data )use($func) {
			        if ( is_array($response_data->caps) ) {
				        $response_data->caps = apply_filters( 'appsbd/vitepos/user/caps', $response_data->caps );
				        if ( empty( $response_data->caps['anbd-discount'] ) ) {
					        $response_data->caps['apbd-wp-login'] = true;
				        }
				        $func($response_data->caps);
			        }
		       ...
		       // } );
		        ...
		        //add_action( 'wp_ajax_vitepos-refresh-license', function () use ( $mainobj ,&$licenseObject,$licenseKey, $liceEmail) {
			     //   if ( __vtpos_css3_handler::refresh_license($licenseKey,$liceEmail,$mainobj->coreObject->plugin_file, $message ,$licenseObject) ) {
				   //     $resp=__vtpos_css3_handler::GetRegisterInfo();
				     //   $info=apply_filters("vitepos/filter/get-license-info",null,$resp);
			           // $info->license_key = substr( $info->license_key, 0, 4 ) . '-XXXX-XXXX' . substr( $info->license_key, -4 );
                    //    wp_send_json($info);;
			        //}
		        //} );
	        }
    ...
    function AdminScript()
    {
        ...
        foreach ($wp_scripts->queue as $script) {
            if ( ! in_array( $script, \VitePos\Core\VitePos::$appsbd_global_js ) ) {
	            if($this->coreObject->is_global_asset($script,$wp_scripts->registered[ $script ]->src )){
		            continue;
	            }
    ...
    function AdminStyle()
    {
        ...
        foreach ( $wp_styles->queue as $style ) {
            if ( ! in_array( $style, \VitePos\Core\VitePos::$appsbd_global_css ) ) {
	            if($this->coreObject->is_global_asset($style,$wp_styles->registered[ $style ]->src)){
		            continue;
	            }
