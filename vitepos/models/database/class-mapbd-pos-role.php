	...
	public static function set_capabilities_by_role( $all_caps, $user ) {
		...
		if ( $user instanceof \WP_User ) {
			...
			foreach ( $user->roles as $role_slug ) {
				$is_administrator=apply_filters('appsbd/vitepos/filter/is-pos-admin','administrator' == $role_slug,$user);
				if ( $is_administrator ) {
		...
		if ( $user instanceof \WP_User ) {
			...
			foreach ( $user->roles as $role_slug ) {
				...
				if ( $is_administrator ) {
					$percentage = 100.00;
					break;
				} elseif ( ! empty( $roles[ $role_slug ] ) && ! empty( $roles[ $role_slug ]->max_discount ) ) {
