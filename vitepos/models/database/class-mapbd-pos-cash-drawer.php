...
class Mapbd_Pos_Cash_Drawer extends ViteposModel {
	 ...
	 * @return bool
	 */
	public function set_close_drawer( $note = 'Drawer closed',$counted_amount=0.0 ) {
		$this_obj = new self();
		$this_obj->status( 'C' );
		$this_obj->counted_amount($counted_amount);
	 ...
	 * @return bool
	 */
	public function set_counted_amount( $amount = 0.0 ) {
		$this_obj = new self();
		if ($this->status=='O' && $amount>0)
		{
			$this_obj->counted_amount( $amount );
			$this_obj->set_where_update( 'id', $this->id );
			if ( $this_obj->update() ) {
				return true;
			}else{
	 ...
	 * @return |null
	 */
	public static function create_by_counter( $opening_balance, $outlet_id, $counter_id, $user_id,$counted_amount=0.0 ) {
		$close_existing = self::get_by_counter( $outlet_id, $counter_id, $user_id );
		if ( $close_existing ) {
			$close_existing->set_close_drawer('Drawer closed',$counted_amount);
		}
		$this_obj = new self();
		$this_obj->outlet_id( $outlet_id );
		$this_obj->counter_id( $counter_id );
	...
	/**
	 * The add cash entry is generated by appsbd
	 *
	 * @param mixed $type It is type param.
	 * @param mixed $narration It is narration param.
	 * @param mixed $amount It is amount param.
	 * @param mixed $cash_drawer It is cash_drawer param.
	 * @param mixed $ref_value It is ref_value param.
	 * @param mixed $ref_type It is ref_type param.
	 * @param mixed $user_note It is user_note param.
	 * @param mixed $extra_param It is extra_param param.
	 *
	 * @return bool
	 */
	public static function add_cash_entry(
		$type,
		$narration,
		$amount,
		$cash_drawer,
	...
	public static function create_db_table() {
		...
		if ( $this_obj->db->get_var( "show tables like '{$table}'" ) != $table ) {
			$sql = "CREATE TABLE `{$table}` (
					  ...
					  `counted_amount` decimal(10,2) DEFAULT '0.00' COMMENT 'This is for enter cash drawer ending amount by cashier',
...
GROUP BY ref_type";
	 ...
	 * @return int
	 */
	public static function get_withdrawn_amount_by_id( $cashdrawer_id, $ref_type = 'W' ) {
		$db       = self::get_db_object();
		$prefix   = $db->prefix;
		$query    = "SELECT SUM(amount) as total
FROM {$prefix}apbd_pos_cash_drawer_log
WHERE cash_drawer_id = $cashdrawer_id and ref_type = '$ref_type'
GROUP BY ref_type";
		$this_obj = new self();
		$results  = $this_obj->select_query( $query );
		if ( ! empty( $results[0]->total ) ) {
