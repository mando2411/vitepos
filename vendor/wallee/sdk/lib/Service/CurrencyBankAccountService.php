...
class CurrencyBankAccountService {
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\Wallee\Sdk\Model\CurrencyBankAccount',
				'/currency-bank-account/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Wallee\Sdk\Model\CurrencyBankAccount', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Wallee\Sdk\Model\EntityQuery $query The query restricts the currency bank accounts which are returned by the search. (required)
	 * @throws \Wallee\Sdk\ApiException
	 * @throws \Wallee\Sdk\VersioningException
	 * @throws \Wallee\Sdk\Http\ConnectionException
	 * @return \Wallee\Sdk\Model\CurrencyBankAccount[]
